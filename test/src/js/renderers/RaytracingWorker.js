...
self.onmessage = function( e ) {
	...
	if ( data.init ) {
		...
		if ( ! renderer ) renderer = new B3D.RaytracingRendererWorker();
		if ( ! loader ) loader = new B3D.ObjectLoader();
	...
	if ( data.scene ) {
		...
		scene.traverse( function( o ) {
			...
			if ( o instanceof B3D.PointLight ) {
...
B3D.RaytracingRendererWorker = function () {

	console.log( 'B3D.RaytracingRendererWorker', B3D.REVISION );
	...
	var origin = new B3D.Vector3();
	var direction = new B3D.Vector3();

	var cameraPosition = new B3D.Vector3();

	var raycaster = new B3D.Raycaster( origin, direction );
	var raycasterLight = new B3D.Raycaster();

	var perspective;
	var modelViewMatrix = new B3D.Matrix4();
	var cameraNormalMatrix = new B3D.Matrix3();
	...
	var spawnRay = ( function () {
		...
		var diffuseColor = new B3D.Color();
		var specularColor = new B3D.Color();
		var lightColor = new B3D.Color();
		var schlick = new B3D.Color();

		var lightContribution = new B3D.Color();

		var eyeVector = new B3D.Vector3();
		var lightVector = new B3D.Vector3();
		var normalVector = new B3D.Vector3();
		var halfVector = new B3D.Vector3();

		var localPoint = new B3D.Vector3();
		var reflectionVector = new B3D.Vector3();

		var tmpVec = new B3D.Vector3();

		var tmpColor = [];

		for ( var i = 0; i < maxRecursionDepth; i ++ ) {

			tmpColor[ i ] = new B3D.Color();
		...
		return function spawnRay( rayOrigin, rayDirection, outputColor, recursionDepth ) {
			...
			if ( material instanceof B3D.MeshLambertMaterial ||
				 material instanceof B3D.MeshPhongMaterial ||
				 material instanceof B3D.MeshBasicMaterial ) {
			...
			if ( material instanceof B3D.MeshBasicMaterial ) {
			...
			} else if ( material instanceof B3D.MeshLambertMaterial ||
						material instanceof B3D.MeshPhongMaterial ) {
				...
				for ( var i = 0, l = lights.length; i < l; i ++ ) {
					...
					if ( material instanceof B3D.MeshPhongMaterial ) {
	...
	var computePixelNormal = ( function () {
		...
		var tmpVec1 = new B3D.Vector3();
		var tmpVec2 = new B3D.Vector3();
		var tmpVec3 = new B3D.Vector3();
		...
		return function computePixelNormal( outputVector, point, shading, face, vertices ) {
			...
			if ( shading === B3D.FlatShading ) {

				outputVector.copy( faceNormal );

			} else if ( shading === B3D.SmoothShading ) {
	...
	this.render = function ( scene, camera ) {
		...
		perspective = 0.5 / Math.tan( B3D.Math.degToRad( camera.fov * 0.5 ) ) * canvasHeight;
		...
		scene.traverse( function ( object ) {
			...
			if ( cache[ object.id ] === undefined ) {
				...
				cache[ object.id ] = {
					normalMatrix: new B3D.Matrix3(),
					inverseMatrix: new B3D.Matrix4()
				};
...
Object.assign( B3D.RaytracingRendererWorker.prototype, B3D.EventDispatcher.prototype );
