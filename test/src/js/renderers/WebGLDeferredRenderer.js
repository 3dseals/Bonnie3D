...
B3D.WebGLDeferredRenderer = function ( parameters ) {
	...
	var _tmpMaterial = new B3D.ShaderMaterial( { visible: false } );
	var _tmpVector3 = new B3D.Vector3();
	...
	function init( parameters ) {
		...
		_this.renderer = parameters.renderer !== undefined ? parameters.renderer : new B3D.WebGLRenderer( { antialias: false } );
	...
	function initDepthTexture() {
		...
		_depthTexture = new B3D.DepthTexture(
			_width,
			_height,
			B3D.UnsignedInt248Type,
			undefined,
			undefined,
			undefined,
			undefined,
	...
	function initPassNormalDepth() {
		...
		_passNormalDepth = new B3D.RenderPass();
		_passNormalDepth.clear = true;

		var rt = new B3D.WebGLRenderTarget( _width, _height, {
		...
		_compNormalDepth = new B3D.EffectComposer( _this.renderer, rt );
	...
	function initPassColor() {
		...
		_compColor = new B3D.EffectComposer( _this.renderer, rt );
	...
	function initPassLight() {
		...
		_passLightFullscreen = new B3D.RenderPass();
		_passLightFullscreen.clear = true;
		_passLightFullscreen.camera = new B3D.OrthographicCamera( - 1, 1, 1, - 1, 0, 1 );

		_passLight = new B3D.RenderPass();
		...
		_compLight = new B3D.EffectComposer( _this.renderer, rt );
	...
	function initPassReconstruction() {
		...
		_passReconstruction = new B3D.RenderPass();
		...
		_compReconstruction = new B3D.EffectComposer( _this.renderer, rt );
	...
	function initPassFinal() {
		...
		_passFinal = new B3D.ShaderPass( B3D.ShaderDeferred[ 'final' ] );
		_passFinal.clear = true;
		_passFinal.uniforms.samplerResult.value = _compLight.renderTarget2.texture;
		_passFinal.material.blending = B3D.NoBlending;
		_passFinal.material.depthWrite = false;
		_passFinal.material.depthTest = false;

		_passForward = new B3D.RenderPass();
		_passForward.clear = false;

		_passCopy = new B3D.ShaderPass( B3D.CopyShader );

		_passFXAA = new B3D.ShaderPass( B3D.FXAAShader );

		var rt = new B3D.WebGLRenderTarget( _width, _height, {
			minFilter: B3D.NearestFilter,
			magFilter: B3D.LinearFilter,
			format: B3D.RGBFormat,
			type: B3D.UnsignedByteType,
			depthTexture: _depthTexture
		} );

		rt.texture.generateMipamps = false;

		_compFinal = new B3D.EffectComposer( _this.renderer, rt );
	...
	function createDeferredNormalDepthMaterial( originalMaterial ) {
		...
		var shader = ( _lightPrePass ) ? B3D.ShaderDeferred[ 'normalDepthShininess' ] : B3D.ShaderDeferred[ 'normalDepth' ];

		return new B3D.ShaderMaterial( {
			uniforms: Object.assign( {}, shader.uniforms ),
			fragmentShader: shader.fragmentShader,
			vertexShader: shader.vertexShader,
			blending: B3D.NoBlending
		} );
	...
	function createDeferredColorMaterial( originalMaterial ) {
		...
		var shader = B3D.ShaderDeferred[ 'color' ];
	...
	function createDeferredReconstructionMaterial( originalMaterial ) {
		...
		var shader = B3D.ShaderDeferred[ 'reconstruction' ];
	...
	function createDeferredEmissiveLight() {
		...
		var shader = B3D.ShaderDeferred[ 'emissiveLight' ];
		...
		var geometry = new B3D.PlaneBufferGeometry( 2, 2 );
		var mesh = new B3D.Mesh( geometry, material );
	...
	function createDeferredLightMesh( light, geometry ) {
		...
		var mesh = new B3D.Mesh( geometry, _tmpMaterial );
	...
	function createDeferredPointLight( light ) {
		...
		var mesh = createDeferredLightMesh( light, new B3D.SphereGeometry( 1, 16, 8 ) );
	...
	function createDeferredPointLightMaterial() {
		...
		var shader = ( _lightPrePass ) ? B3D.ShaderDeferred[ 'pointLightPre' ] : B3D.ShaderDeferred[ 'pointLight' ];

		var material = createDeferredLightShaderMaterial( shader );

		material.side = B3D.BackSide;
		material.depthFunc = B3D.GreaterEqualDepth;
	...
	function createDeferredSpotLight( light ) {
		...
		var mesh = createDeferredLightMesh( light, new B3D.PlaneBufferGeometry( 2, 2 ) );
	...
	function createDeferredSpotLightMaterial() {
		...
		var shader = ( _lightPrePass ) ? B3D.ShaderDeferred[ 'spotLightPre' ] : B3D.ShaderDeferred[ 'spotLight' ];
	...
	function createDeferredDirectionalLightMaterial() {
		...
		var shader = ( _lightPrePass ) ? B3D.ShaderDeferred[ 'directionalLightPre' ] : B3D.ShaderDeferred[ 'directionalLight' ];
	...
	function updateDeferredCommonUniforms( camera ) {
		...
		var uniforms = B3D.ShaderDeferredCommon[ 'commonUniforms' ];
...
B3D.ShaderDeferredCommon = {

	commonUniforms: {

		matProjInverse: new B3D.Uniform( new B3D.Matrix4() ),

		viewWidth: new B3D.Uniform( 800 ),
		viewHeight: new B3D.Uniform( 600 )
...
B3D.ShaderDeferred = {
	...
	normalDepth: {
		...
		fragmentShader: [
			...
			B3D.DeferredShaderChunk[ "packNormalDepth" ],
	...
	color: {
		...
		uniforms: {
			...
			map: new B3D.Uniform( null ),
			offsetRepeat: new B3D.Uniform( new B3D.Vector4( 0, 0, 1, 1 ) ),

			diffuse: new B3D.Uniform( new B3D.Color( 0x000000 ) ),
			emissive: new B3D.Uniform( new B3D.Color( 0x000000 ) ),
			specular: new B3D.Uniform( new B3D.Color( 0x000000 ) ),
			shininess: new B3D.Uniform( 30.0 )

		},

		vertexShader: [

			B3D.ShaderChunk[ "uv_pars_vertex" ],
			B3D.ShaderChunk[ "morphtarget_pars_vertex" ],
			B3D.ShaderChunk[ "skinning_pars_vertex" ],

			"void main() {",

			B3D.ShaderChunk[ "uv_vertex" ],
			B3D.ShaderChunk[ "begin_vertex" ],
			B3D.ShaderChunk[ "beginnormal_vertex" ],
			B3D.ShaderChunk[ "skinbase_vertex" ],
			B3D.ShaderChunk[ "skinnormal_vertex" ],
		...
		fragmentShader: [
			...
			B3D.ShaderChunk[ "uv_pars_fragment" ],
			B3D.ShaderChunk[ "map_pars_fragment" ],
			B3D.DeferredShaderChunk[ "packVector3" ],
			...
			B3D.ShaderChunk[ "map_fragment" ],
			B3D.DeferredShaderChunk[ "packColor" ],
	...
	emissiveLight: {
		...
		uniforms: Object.assign(

			{
				...
				samplerColor: new B3D.Uniform( null )

			},

			B3D.ShaderDeferredCommon[ 'commonUniforms' ]
		...
		fragmentShader: [
			...
			B3D.DeferredShaderChunk[ "unpackFloat" ],

			"void main() {",

			B3D.DeferredShaderChunk[ "computeTextureCoord" ],
			B3D.DeferredShaderChunk[ "unpackColor" ],
	...
	pointLight: {
		...
		uniforms: Object.assign(

			{
				...
				samplerNormalDepth: new B3D.Uniform( null ),
				samplerColor: new B3D.Uniform( null ),

				lightColor: new B3D.Uniform( new B3D.Color( 0x000000 ) ),
				lightPositionVS: new B3D.Uniform( new B3D.Vector3( 0, 1, 0 ) ),
				lightIntensity: new B3D.Uniform( 1.0 ),
				lightRadius: new B3D.Uniform( 1.0 )
		...
		fragmentShader: [
			...
			B3D.DeferredShaderChunk[ "computeTextureCoord" ],
			B3D.DeferredShaderChunk[ "unpackNormalDepth" ],
			B3D.DeferredShaderChunk[ "computeVertexPositionVS" ],
			...
			B3D.DeferredShaderChunk[ "unpackColor" ],
			B3D.DeferredShaderChunk[ "computeSpecular" ],
			...
			B3D.DeferredShaderChunk[ "combine" ],
	...
	spotLight: {
		...
		uniforms: Object.assign(

			{
				...
				lightColor: new B3D.Uniform( new B3D.Color( 0x000000 ) ),
				lightDirectionVS: new B3D.Uniform( new B3D.Vector3( 0, 1, 0 ) ),
				lightPositionVS: new B3D.Uniform( new B3D.Vector3( 0, 1, 0 ) ),
				lightAngle: new B3D.Uniform( 1.0 ),
				lightIntensity: new B3D.Uniform( 1.0 )
	...
	normalDepthShininess: {
		...
		fragmentShader: [
			...
			B3D.DeferredShaderChunk[ "packNormal" ],
			...
			B3D.DeferredShaderChunk[ "packNormalDepthShininess" ],
	...
	pointLightPre: {
		...
		uniforms: Object.assign(

			{
				...
				samplerNormalDepthShininess: new B3D.Uniform( null ),
		...
		fragmentShader: [
			...
			B3D.DeferredShaderChunk[ "unpackFloat" ],
			B3D.DeferredShaderChunk[ "unpackVector2" ],

			"void main() {",

			B3D.DeferredShaderChunk[ "computeTextureCoord" ],
			B3D.DeferredShaderChunk[ "unpackNormalDepthShininess" ],
			B3D.DeferredShaderChunk[ "computeVertexPositionVS" ],
			...
			B3D.DeferredShaderChunk[ "packLight" ],
	...
	reconstruction: {
		...
		uniforms: Object.assign(

			{
				...
				samplerLight: new B3D.Uniform( null ),
	...
	final: {
		...
		uniforms: {
			...
			samplerResult: new B3D.Uniform( null )
