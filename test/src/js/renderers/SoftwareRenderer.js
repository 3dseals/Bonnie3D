...
B3D.SoftwareRenderer = function ( parameters ) {

	console.log( 'B3D.SoftwareRenderer', B3D.REVISION );
	...
	var clearColor = new B3D.Color( 0x000000 );
	...
	var lineMode = false;
	var lookVector = new B3D.Vector3( 0, 0, 1 );
	var crossVector = new B3D.Vector3();
	...
	var projector = new B3D.Projector();

	var spriteV1 = new B3D.Vector4();
	var spriteV2 = new B3D.Vector4();
	var spriteV3 = new B3D.Vector4();

	var spriteUV1 = new B3D.Vector2();
	var spriteUV2 = new B3D.Vector2();
	var spriteUV3 = new B3D.Vector2();
	...
	this.render = function ( scene, camera ) {
		...
		for ( var e = 0, el = elements.length; e < el; e ++ ) {
			...
			if ( element instanceof B3D.RenderableFace ) {
			...
			} else if ( element instanceof B3D.RenderableSprite ) {
	...
	function getMaterialShader( material ) {
		...
		if ( shaders[ id ] === undefined || material.needsUpdate === true ) {
			...
			if ( material instanceof B3D.MeshBasicMaterial ||
				material instanceof B3D.MeshLambertMaterial ||
				material instanceof B3D.MeshPhongMaterial ||
				material instanceof B3D.SpriteMaterial ) {
				...
				} else if ( material instanceof B3D.MeshPhongMaterial ) {
				...
				if ( material.map ) {
					...
					var texture = new B3D.SoftwareRenderer.Texture();
			...
			} else if ( material instanceof B3D.LineBasicMaterial ) {
	...
	*/
	...
	function drawTriangle( v1, v2, v3, uv1, uv2, uv3, shader, face, material ) {
		...
		if ( ! ( face instanceof B3D.RenderableSprite ) && ( longestSide > 100 * fixscale ) ) {
			...
			if ( bHasUV ) {
				...
				if ( mpUVPoolCount === mpUVPool.length ) {
					...
					mpUV12 = new B3D.Vector2();
					mpUVPool.push( mpUV12 );
					++mpUVPoolCount;

					mpUV23 = new B3D.Vector2();
					mpUVPool.push( mpUV23 );
					++mpUVPoolCount;

					mpUV31 = new B3D.Vector2();
			...
			if ( mpVPoolCount === mpVPool.length ) {
				...
				mpV12 = new B3D.Vector4();
				mpVPool.push( mpV12 );
				++mpVPoolCount;

				mpV23 = new B3D.Vector4();
				mpVPool.push( mpV23 );
				++mpVPoolCount;

				mpV31 = new B3D.Vector4();
				mpVPool.push( mpV31 );
				++mpVPoolCount;
			...
			if ( bHasNormal ) {
				...
				if ( mpNPoolCount === mpNPool.length ) {
					...
					mpN12 = new B3D.Vector3();
					mpNPool.push( mpN12 );
					++mpNPoolCount;

					mpN23 = new B3D.Vector3();
					mpNPool.push( mpN23 );
					++mpNPoolCount;

					mpN31 = new B3D.Vector3();
					mpNPool.push( mpN31 );
					++mpNPoolCount;
...
B3D.SoftwareRenderer.Texture = function () {
	...
	this.fromImage = function ( image ) {
		...
		size = B3D.Math.nextPowerOfTwo( size );
