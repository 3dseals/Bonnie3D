...
B3D.RaytracingRenderer = function ( parameters ) {

	console.log( 'B3D.RaytracingRenderer', B3D.REVISION );
	...
	this.setWorkers = function( w ) {
		...
		while ( pool.length < workers ) {
			...
			worker.color = new B3D.Color().setHSL( Math.random() , 0.8, 0.8 ).getHexString();
			pool.push( worker );
...
Object.assign( B3D.RaytracingRenderer.prototype, B3D.EventDispatcher.prototype );
