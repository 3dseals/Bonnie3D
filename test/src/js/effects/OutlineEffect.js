 ...
 * new B3D.OutlineEffect( renderer, {
 * 	defaultThickNess: 0.01,
 * 	defaultColor: new B3D.Color( 0x888888 ),
 * 	defaultAlpha: 0.8,
...
B3D.OutlineEffect = function ( renderer, parameters ) {
	...
	var defaultColor = parameters.defaultColor !== undefined ? parameters.defaultColor : new B3D.Color( 0x000000 );
	...
	function createInvisibleMaterial() {
		...
		return new B3D.ShaderMaterial( { name: 'invisible', visible: false } );
	...
	function createMaterial( originalMaterial ) {
		...
		if ( shaderID !== undefined ) {
			...
			var shader = B3D.ShaderLib[ shaderID ];
			...
			if ( ! /attribute\s+vec3\s+position\s*;/.test( originalVertexShader ) ||
				...
				console.warn( 'B3D.OutlineEffect requires both vec3 position and normal attributes in vertex shader, ' +
	 ...
	 * B3D.OutlineEffect to be called from other *Effect, like
	 *
	 * effect = new B3D.VREffect( new B3D.OutlineEffect( renderer ) );
	 *
	 * function render () {
	 *
