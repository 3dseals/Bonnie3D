 ...
 *
 * See B3D.MMDLoader for the passed parameter list of RigidBody/Constraint.
 *
 * Requirement:
...
B3D.MMDPhysics = function ( mesh, params ) {

	if ( params === undefined ) params = {};

	this.mesh = mesh;
	this.helper = new B3D.MMDPhysics.ResourceHelper();
...
B3D.MMDPhysics.prototype = {

	constructor: B3D.MMDPhysics,
	...
	initRigidBodies: function () {
		...
		for ( var i = 0; i < bodies.length; i++ ) {
			...
			var b = new B3D.MMDPhysics.RigidBody( this.mesh, this.world, bodies[ i ], this.helper );
			this.bodies.push( b );
	...
	initConstraints: function () {
		...
		for ( var i = 0; i < constraints.length; i++ ) {
			...
			var c = new B3D.MMDPhysics.Constraint( this.mesh, this.world, bodyA, bodyB, params, this.helper );
 ...
 */
B3D.MMDPhysics.ResourceHelper = function () {
...
B3D.MMDPhysics.ResourceHelper.prototype = {

	allocThreeVector3: function () {

		return ( this.threeVector3s.length > 0 ) ? this.threeVector3s.pop() : new B3D.Vector3();
	...
	allocThreeMatrix4: function () {
		...
		return ( this.threeMatrix4s.length > 0 ) ? this.threeMatrix4s.pop() : new B3D.Matrix4();
	...
	allocThreeQuaternion: function () {
		...
		return ( this.threeQuaternions.length > 0 ) ? this.threeQuaternions.pop() : new B3D.Quaternion();
	...
	allocThreeEuler: function () {
		...
		return ( this.threeEulers.length > 0 ) ? this.threeEulers.pop() : new B3D.Euler();
...
B3D.MMDPhysics.RigidBody = function ( mesh, world, params, helper ) {
...
B3D.MMDPhysics.RigidBody.prototype = {

	constructor: B3D.MMDPhysics.RigidBody,
	...
	init: function () {
		...
		var bone = ( params.boneIndex === -1 ) ? new B3D.Bone() : bones[ params.boneIndex ];
...
B3D.MMDPhysics.Constraint = function ( mesh, world, bodyA, bodyB, params, helper ) {
...
B3D.MMDPhysics.Constraint.prototype = {

	constructor: B3D.MMDPhysics.Constraint,
...
B3D.MMDPhysicsHelper = function ( mesh ) {

	if ( mesh.physics === undefined || mesh.geometry.rigidBodies === undefined ) {

		throw 'B3D.MMDPhysicsHelper requires physics in mesh and rigidBodies in mesh.geometry.';
	...
	this.materials.push(
		new B3D.MeshBasicMaterial( {
			color: new B3D.Color( 0xff8888 ),
			wireframe: true,
			depthTest: false,
			depthWrite: false,
			opacity: 0.25,
...
B3D.MMDPhysicsHelper.prototype = Object.create( B3D.Object3D.prototype );
B3D.MMDPhysicsHelper.prototype.constructor = B3D.MMDPhysicsHelper;

B3D.MMDPhysicsHelper.prototype._init = function () {
	...
	function createGeometry( param ) {
		...
		switch ( param.shapeType ) {
			...
			case 0:
				return new B3D.SphereBufferGeometry( param.width, 16, 8 );

			case 1:
				return new B3D.BoxBufferGeometry( param.width * 2, param.height * 2, param.depth * 2, 8, 8, 8);
	...
	function createCapsuleGeometry( radius, cylinderHeight, segmentsRadius, segmentsHeight ) {
		...
		var geometry = new B3D.CylinderBufferGeometry( radius, radius, cylinderHeight, segmentsRadius, segmentsHeight, true );
		var upperSphere = new B3D.Mesh( new B3D.SphereBufferGeometry( radius, segmentsRadius, segmentsHeight, 0, Math.PI * 2, 0, Math.PI / 2 ) );
		var lowerSphere = new B3D.Mesh( new B3D.SphereBufferGeometry( radius, segmentsRadius, segmentsHeight, 0, Math.PI * 2, Math.PI / 2, Math.PI / 2 ) );
	...
	for ( var i = 0, il = rigidBodies.length; i < il; i ++ ) {
		...
		this.add( new B3D.Mesh( createGeometry( param ), this.materials[ param.type ] ) );

	}

};

B3D.MMDPhysicsHelper.prototype.update = function () {

	var mesh = this.root;
	var rigidBodies = mesh.geometry.rigidBodies;
	var bodies = mesh.physics.bodies;

	var matrixWorldInv = new B3D.Matrix4().getInverse( mesh.matrixWorld );
	var vector = new B3D.Vector3();
	var quaternion = new B3D.Quaternion();
	var quaternion2 = new B3D.Quaternion();
