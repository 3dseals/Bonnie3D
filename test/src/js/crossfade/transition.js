function Transition ( sceneA, sceneB ) {
	...
	this.cameraOrtho = new B3D.OrthographicCamera( window.innerWidth / - 2, window.innerWidth / 2, window.innerHeight / 2, window.innerHeight / - 2, - 10, 10 );

	this.textures = [];

	var loader = new B3D.TextureLoader();

	for ( var i = 0; i < 6; i ++ )
		this.textures[ i ] = loader.load( 'textures/transition/transition' + ( i + 1 ) + '.png' );

	this.quadmaterial = new B3D.ShaderMaterial( {
	...
	var quadgeometry = new B3D.PlaneBufferGeometry( window.innerWidth, window.innerHeight );

	this.quad = new B3D.Mesh( quadgeometry, this.quadmaterial );
	...
	this.render = function( delta ) {
		...
		if ( transitionParams.animateTransition ) {
			...
			transitionParams.transition = B3D.Math.smoothstep( t, 0.3, 0.7 );
