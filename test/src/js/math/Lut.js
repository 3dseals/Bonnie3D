...
B3D.Lut = function ( colormap, numberofcolors ) {

	this.lut = [];
	this.map = B3D.ColorMapKeywords[ colormap ];
	...
	for ( var i = 0; i <= 1; i += step ) {
		...
		for ( var j = 0; j < this.map.length - 1; j ++ ) {
			...
			if ( i >= this.map[ j ][ 0 ] && i < this.map[ j + 1 ][ 0 ] ) {
				...
				var minColor = new B3D.Color( 0xffffff ).setHex( this.map[ j ][ 1 ] );
				var maxColor = new B3D.Color( 0xffffff ).setHex( this.map[ j + 1 ][ 1 ] );
...
B3D.Lut.prototype = {
	...
	changeNumberOfColors: function ( numberofcolors ) {
		...
		return new B3D.Lut( this.mapname, this.n );
	...
	addColorMap: function ( colormapName, arrayOfColors ) {
		...
		B3D.ColorMapKeywords[ colormapName ] = arrayOfColors;
	...
	setLegendOn: function ( parameters ) {
		...
		this.legend.texture = new B3D.Texture( this.legend.canvas );

		var imageData = this.legend.ctx.getImageData( 0, 0, 1, this.n );

		var data = imageData.data;

		this.map = B3D.ColorMapKeywords[ this.mapname ];
		...
		for ( var i = 1; i >= 0; i -= step ) {
			...
			for ( var j = this.map.length - 1; j >= 0; j -- ) {
				...
				if ( i < this.map[ j ][ 0 ] && i >= this.map[ j - 1 ][ 0 ]  ) {
					...
					var minColor = new B3D.Color( 0xffffff ).setHex( this.map[ j - 1 ][ 1 ] );
					var maxColor = new B3D.Color( 0xffffff ).setHex( this.map[ j ][ 1 ] );
		...
		this.legend.legendGeometry = new B3D.PlaneBufferGeometry( this.legend.dimensions.width, this.legend.dimensions.height );
		this.legend.legendMaterial = new B3D.MeshBasicMaterial( { map : this.legend.texture, side : B3D.DoubleSide } );

		this.legend.mesh = new B3D.Mesh( this.legend.legendGeometry, this.legend.legendMaterial );
	...
	setLegendPosition: function ( position ) {
		...
		this.legend.position = new B3D.Vector3( position.x, position.y, position.z );
	...
	setLegendLabels: function ( parameters, callback ) {
		...
		var txtTitle = new B3D.CanvasTexture( canvasTitle );
		txtTitle.minFilter = B3D.LinearFilter;

		var spriteMaterialTitle = new B3D.SpriteMaterial( { map: txtTitle } );

		var spriteTitle = new B3D.Sprite( spriteMaterialTitle );
		...
		if ( this.legend.labels.ticks > 0 ) {
			...
			for ( var i = 0; i < this.legend.labels.ticks; i ++ ) {
				...
				var txtTick = new B3D.CanvasTexture( canvasTick );
				txtTick.minFilter = B3D.LinearFilter;

				var spriteMaterialTick = new B3D.SpriteMaterial( { map: txtTick } );

				var spriteTick = new B3D.Sprite( spriteMaterialTick );
				...
				var material = new B3D.LineBasicMaterial( { color: 0x000000, linewidth: 2 } );
				...
				if ( this.legend.layout == 'vertical' ) {
					...
					geometry.vertices.push( new B3D.Vector3( this.legend.position.x + this.legend.dimensions.width * 0.55, linePosition, this.legend.position.z  ) );

					geometry.vertices.push( new B3D.Vector3( this.legend.position.x + this.legend.dimensions.width * 0.7, linePosition, this.legend.position.z  ) );
				...
				if ( this.legend.layout == 'horizontal' ) {
					...
					geometry.vertices.push( new B3D.Vector3( linePosition, this.legend.position.y - this.legend.dimensions.width * 0.55, this.legend.position.z  ) );

					geometry.vertices.push( new B3D.Vector3( linePosition, this.legend.position.y - this.legend.dimensions.width * 0.7, this.legend.position.z  ) );

				}

				var line = new B3D.Line( geometry, material );
