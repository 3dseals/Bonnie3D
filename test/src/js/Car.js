...
B3D.Car = function () {
	...
	this.wheelOffset = new B3D.Vector3();
	...
	this.root = new B3D.Object3D();

	this.frontLeftWheelRoot = new B3D.Object3D();
	this.frontRightWheelRoot = new B3D.Object3D();
	...
	this.loadPartsBinary = function ( bodyURL, wheelURL ) {
		...
		var loader = new B3D.BinaryLoader();
	...
	this.updateCarModel = function ( delta, controls ) {
		...
		if ( controls.moveForward ) {
			...
			this.speed = B3D.Math.clamp( this.speed + delta * this.FRONT_ACCELERATION, this.MAX_REVERSE_SPEED, this.MAX_SPEED );
			this.acceleration = B3D.Math.clamp( this.acceleration + delta, - 1, 1 );
		...
		if ( controls.moveBackward ) {
			...
			this.speed = B3D.Math.clamp( this.speed - delta * this.BACK_ACCELERATION, this.MAX_REVERSE_SPEED, this.MAX_SPEED );
			this.acceleration = B3D.Math.clamp( this.acceleration - delta, - 1, 1 );
		...
		if ( controls.moveLeft ) {
			...
			this.wheelOrientation = B3D.Math.clamp( this.wheelOrientation + delta * this.WHEEL_ANGULAR_ACCELERATION, - this.MAX_WHEEL_ROTATION, this.MAX_WHEEL_ROTATION );
		...
		if ( ! ( controls.moveForward || controls.moveBackward ) ) {
			...
			if ( this.speed > 0 ) {
				...
				this.speed = B3D.Math.clamp( this.speed - k * delta * this.FRONT_DECCELERATION, 0, this.MAX_SPEED );
				this.acceleration = B3D.Math.clamp( this.acceleration - k * delta, 0, 1 );

			} else {

				var k = exponentialEaseOut( this.speed / this.MAX_REVERSE_SPEED );

				this.speed = B3D.Math.clamp( this.speed + k * delta * this.BACK_ACCELERATION, this.MAX_REVERSE_SPEED, 0 );
				this.acceleration = B3D.Math.clamp( this.acceleration + k * delta, - 1, 0 );
		...
		if ( ! ( controls.moveLeft || controls.moveRight ) ) {
			...
			if ( this.wheelOrientation > 0 ) {
				...
				this.wheelOrientation = B3D.Math.clamp( this.wheelOrientation - delta * this.WHEEL_ANGULAR_DECCELERATION, 0, this.MAX_WHEEL_ROTATION );

			} else {

				this.wheelOrientation = B3D.Math.clamp( this.wheelOrientation + delta * this.WHEEL_ANGULAR_DECCELERATION, - this.MAX_WHEEL_ROTATION, 0 );
	...
	function createCar () {
		...
		if ( scope.bodyGeometry && scope.wheelGeometry ) {
			...
			scope.bodyMesh = new B3D.Mesh( scope.bodyGeometry, bodyFaceMaterial );
			...
			delta.multiplyVectors( scope.wheelOffset, new B3D.Vector3( s, s, s ) );

			scope.frontLeftWheelRoot.position.add( delta );

			scope.frontLeftWheelMesh = new B3D.Mesh( scope.wheelGeometry, wheelFaceMaterial );
			...
			scope.frontRightWheelMesh = new B3D.Mesh( scope.wheelGeometry, wheelFaceMaterial );
			...
			scope.backLeftWheelMesh = new B3D.Mesh( scope.wheelGeometry, wheelFaceMaterial );
			...
			scope.backRightWheelMesh = new B3D.Mesh( scope.wheelGeometry, wheelFaceMaterial );
