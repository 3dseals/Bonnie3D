 ...
 */
 ( function ( B3D ) {
	...
	B3D.Octree = function ( parameters ) {
		...
		this.utilVec31Search = new B3D.Vector3();
		this.utilVec32Search = new B3D.Vector3();
		...
		if ( this.scene ) {
			...
			this.visualGeometry = new B3D.BoxGeometry( 1, 1, 1 );
			this.visualMaterial = new B3D.MeshBasicMaterial( { color: 0xFF0066, wireframe: true, wireframeLinewidth: 1 } );
		...
		this.root = parameters.root instanceof B3D.OctreeNode ? parameters.root : new B3D.OctreeNode( parameters );
		
	};

	B3D.Octree.prototype = {
		...
		addDeferred: function ( object, options ) {
			...
			if ( object instanceof B3D.OctreeObjectData ) {
			...
			if ( ! object.uuid ) {
				...
				object.uuid = B3D.Math.generateUUID();
		...
		addObjectData: function ( object, part ) {
			...
			var objectData = new B3D.OctreeObjectData( object, part );
		...
		extend: function ( octree ) {
			...
			if ( octree instanceof B3D.Octree ) {
		...
		rebuild: function () {
			...
			for ( i = 0, l = this.objectsData.length; i < l; i ++ ) {
				...
				if ( node instanceof B3D.OctreeNode && ! objectData.positionLast.equals( objectData.position ) ) {
		...
		search: function ( position, radius, organizeByObject, direction ) {
			...
			if ( direction instanceof B3D.Vector3 ) {
		...
		setRoot: function ( root ) { 
			...
			if ( root instanceof B3D.OctreeNode ) {
	...
	B3D.OctreeObjectData = function ( object, part ) {
		...
		if ( part instanceof B3D.Face3 ) {
			
			this.faces = part;
			this.face3 = true;
			this.utilVec31FaceBounds = new B3D.Vector3();
			
		} else if ( part instanceof B3D.Vector3 ) {
		...
		if ( this.object instanceof B3D.Object3D ) {
	...
	B3D.OctreeObjectData.prototype = {
		...
		getFace3BoundingRadius: function ( object, face ) {
			...
			if ( face.centroid === undefined ) face.centroid = new B3D.Vector3();
	...
	B3D.OctreeNode = function ( parameters ) {
		
		// utility
		
		this.utilVec31Branch = new B3D.Vector3();
		this.utilVec31Expand = new B3D.Vector3();
		this.utilVec31Ray = new B3D.Vector3();
		...
		if ( parameters.tree instanceof B3D.Octree ) {
			
			this.tree = parameters.tree;
			
		} else if ( parameters.parent instanceof B3D.OctreeNode !== true ) {
			
			parameters.root = this;
			
			this.tree = new B3D.Octree( parameters );
		...
		this.position = parameters.position instanceof B3D.Vector3 ? parameters.position : new B3D.Vector3();
		...
		if ( this.tree.scene ) {
			...
			this.visual = new B3D.Mesh( this.tree.visualGeometry, this.tree.visualMaterial );
	...
	B3D.OctreeNode.prototype = {
		...
		updateProperties: function () {
			...
			if ( this.parent instanceof B3D.OctreeNode ) {
		...
		addObject: function ( object ) {
			...
			} else if ( indexOctant < - 1 && this.parent instanceof B3D.OctreeNode ) {
		...
		branch: function ( indexOctant ) {
			...
			if ( this.nodesByIndex[ indexOctant ] instanceof B3D.OctreeNode ) {
				...
				position = new B3D.Vector3().addVectors( this.position, offset );
				
				// node
				
				node = new B3D.OctreeNode( {
					tree: this.tree,
					parent: this,
					position: position,
					radius: radius,
		...
		checkMerge: function () {
			...
			while ( nodeParent.parent instanceof B3D.OctreeNode && nodeParent.getObjectCountEnd() < this.tree.objectsThreshold ) {
		...
		checkContract: function () {
			...
			if ( this.nodesIndices.length > 0 ) {
				...
				for ( i = 0, l = this.nodesIndices.length; i < l; i ++ ) {
					...
					if ( nodeHeaviest instanceof B3D.OctreeNode === false || nodeObjectsCount > nodeHeaviestObjectsCount ) {
				...
				if ( outsideHeaviestObjectsCount < this.tree.objectsThreshold && nodeHeaviest instanceof B3D.OctreeNode ) {
		...
		getOctantIndex: function ( objectData ) {
			...
			if ( objectData instanceof B3D.OctreeObjectData ) {
			...
			} else if ( objectData instanceof B3D.OctreeNode ) {
		...
		getObjectCountStart: function () {
			...
			while ( parent instanceof B3D.OctreeNode ) {
	...
	B3D.Raycaster.prototype.intersectOctreeObject = function ( object, recursive ) {
		...
		if ( object.object instanceof B3D.Object3D ) {
	...
	B3D.Raycaster.prototype.intersectOctreeObjects = function ( objects, recursive ) {
