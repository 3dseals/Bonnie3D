...
B3D.GPUParticleSystem = function( options ) {

	B3D.Object3D.apply( this, arguments );
	...
	var textureLoader = new B3D.TextureLoader();

	this.particleNoiseTex = this.PARTICLE_NOISE_TEXTURE || textureLoader.load( 'textures/perlin-512.png' );
	this.particleNoiseTex.wrapS = this.particleNoiseTex.wrapT = B3D.RepeatWrapping;

	this.particleSpriteTex = this.PARTICLE_SPRITE_TEXTURE || textureLoader.load( 'textures/particle2.png' );
	this.particleSpriteTex.wrapS = this.particleSpriteTex.wrapT = B3D.RepeatWrapping;

	this.particleShaderMat = new B3D.ShaderMaterial( {
		transparent: true,
		depthWrite: false,
		uniforms: {
			'uTime': {
	...
	this.init = function() {
		...
		for ( var i = 0; i < this.PARTICLE_CONTAINERS; i ++ ) {
			...
			var c = new B3D.GPUParticleContainer( this.PARTICLES_PER_CONTAINER, this );
...
B3D.GPUParticleSystem.prototype = Object.create( B3D.Object3D.prototype );
B3D.GPUParticleSystem.prototype.constructor = B3D.GPUParticleSystem;


// Subclass for particle containers, allows for very large arrays to be spread out

B3D.GPUParticleContainer = function( maxParticles, particleSystem ) {
	...
	this.particleShaderGeo = new B3D.BufferGeometry();

	this.particleShaderGeo.addAttribute( 'position', new B3D.BufferAttribute( new Float32Array( this.PARTICLE_COUNT * 3 ), 3 ).setDynamic( true ) );
	this.particleShaderGeo.addAttribute( 'positionStart', new B3D.BufferAttribute( new Float32Array( this.PARTICLE_COUNT * 3 ), 3 ).setDynamic( true ) );
	this.particleShaderGeo.addAttribute( 'startTime', new B3D.BufferAttribute( new Float32Array( this.PARTICLE_COUNT ), 1 ).setDynamic( true ) );
	this.particleShaderGeo.addAttribute( 'velocity', new B3D.BufferAttribute( new Float32Array( this.PARTICLE_COUNT * 3 ), 3 ).setDynamic( true ) );
	this.particleShaderGeo.addAttribute( 'turbulence', new B3D.BufferAttribute( new Float32Array( this.PARTICLE_COUNT ), 1 ).setDynamic( true ) );
	this.particleShaderGeo.addAttribute( 'color', new B3D.BufferAttribute( new Float32Array( this.PARTICLE_COUNT * 3 ), 3 ).setDynamic( true ) );
	this.particleShaderGeo.addAttribute( 'size', new B3D.BufferAttribute( new Float32Array( this.PARTICLE_COUNT ), 1 ).setDynamic( true ) );
	this.particleShaderGeo.addAttribute( 'lifeTime', new B3D.BufferAttribute( new Float32Array( this.PARTICLE_COUNT ), 1 ).setDynamic( true ) );
	...
	var velocity = new B3D.Vector3();
	var color = new B3D.Color();
	...
	this.spawnParticle = function( options ) {
		...
		velX = B3D.Math.clamp( ( velX - ( - maxVel ) ) / ( maxVel - ( - maxVel ) ), 0, 1 );
		velY = B3D.Math.clamp( ( velY - ( - maxVel ) ) / ( maxVel - ( - maxVel ) ), 0, 1 );
		velZ = B3D.Math.clamp( ( velZ - ( - maxVel ) ) / ( maxVel - ( - maxVel ) ), 0, 1 );
		...
		color.r = B3D.Math.clamp( color.r + particleSystem.random() * colorRandomness, 0, 1 );
		color.g = B3D.Math.clamp( color.g + particleSystem.random() * colorRandomness, 0, 1 );
		color.b = B3D.Math.clamp( color.b + particleSystem.random() * colorRandomness, 0, 1 );
	...
	this.init = function() {
		...
		this.particleSystem = new B3D.Points( this.particleShaderGeo, this.particleShaderMat );
...
B3D.GPUParticleContainer.prototype = Object.create( B3D.Object3D.prototype );
B3D.GPUParticleContainer.prototype.constructor = B3D.GPUParticleContainer;
