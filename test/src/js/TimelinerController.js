...
B3D.TimelinerController = function TimelinerController( scene, trackInfo, onUpdate ) {
	...
	this._mixer = new B3D.AnimationMixer( scene );
	this._clip = null;
	this._action = null;
...
B3D.TimelinerController.prototype = {

	constructor: B3D.TimelinerController,
	...
	init: function( timeliner ) {
		...
		this._clip = new B3D.AnimationClip( 'editclip', 0, tracks );
	...
	_sort: function( track ) {
		...
		var times = track.times,
			order = B3D.AnimationUtils.getKeyframeOrder( times );

		this._setArray( times,
				B3D.AnimationUtils.sortedArray( times, 1, order ) );

		var values = track.values,
			stride = track.getValueSize();

		this._setArray( values,
				B3D.AnimationUtils.sortedArray( values, stride, order ) );
	...
	_addTrack: function( type, prop, initialValue, interpolation ) {
		...
		this._propRefs[ prop ] =
				new B3D.PropertyBinding( this._scene, prop );
