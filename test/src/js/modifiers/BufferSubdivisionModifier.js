 ...
 *
 * @input B3D.Geometry, or index'd B3D.BufferGeometry with faceUV's (Not vertex uv's)
...
B3D.Face3.prototype.set = function( a, b, c ) {
...
TypedArrayHelper.prototype = {
	...
	index_to_register: function( index, register, isLoop ) {
		...
		if ( register >= this.available_registers ) {
			...
			throw new Error( 'B3D.BufferSubdivisionModifier: Not enough registers in TypedArrayHelper.' );

		}

		if ( index > this.length ) {

			throw new Error( 'B3D.BufferSubdivisionModifier: Index is out of range in TypedArrayHelper.' );
...
function convertGeometryToIndexedBuffer( geometry ) {
	...
	var BGeom = new B3D.BufferGeometry();

	// create a new typed array
	var vertArray = new TypedArrayHelper( geometry.vertices.length, 0, B3D.Vector3, Float32Array, 3, [ 'x', 'y', 'z' ] );
	var indexArray = new TypedArrayHelper( geometry.faces.length, 0, B3D.Face3, Uint32Array, 3, [ 'a', 'b', 'c' ] );
	var uvArray = new TypedArrayHelper( geometry.faceVertexUvs[0].length * 3 * 3, 0, B3D.Vector2, Float32Array, 2, [ 'x', 'y' ] );
	...
	BGeom.setIndex( new B3D.BufferAttribute( indexArray.buffer, 3 ) );
	BGeom.addAttribute( 'position', new B3D.BufferAttribute( vertArray.buffer, 3 ) );
	BGeom.addAttribute( 'uv', new B3D.BufferAttribute( uvArray.buffer, 2 ) );
...
function compute_vertex_normals( geometry ) {
	...
	var oldVertices = new TypedArrayHelper( 0, 5, B3D.Vector3, Float32Array, 3, XYZ );
	var oldFaces = new TypedArrayHelper( 0, 3, B3D.Face3, Uint32Array, 3, ABC );
	oldVertices.from_existing( geometry.getAttribute( 'position' ).array );
	var newNormals = new TypedArrayHelper( oldVertices.length * 3, 4, B3D.Vector3, Float32Array, 3, XYZ );
	...
	geometry.addAttribute( 'normal', new B3D.BufferAttribute( newNormals.buffer, 3 ) );
...
function unIndexIndexedGeometry( geometry ) {
	...
	var oldVertices = new TypedArrayHelper( 0, 3, B3D.Vector3, Float32Array, 3, XYZ );
	var oldFaces = new TypedArrayHelper( 0, 3, B3D.Face3, Uint32Array, 3, ABC );
	var oldUvs = new TypedArrayHelper( 0, 3, B3D.Vector2, Float32Array, 2, XY );
	var oldNormals = new TypedArrayHelper( 0, 3, B3D.Vector3, Float32Array, 3, XYZ );
	...
	var newVertices = new TypedArrayHelper( oldFaces.length * 3, 3, B3D.Vector3, Float32Array, 3, XYZ );
	var newNormals = new TypedArrayHelper( oldFaces.length * 3, 3, B3D.Vector3, Float32Array, 3, XYZ );
	var newUvs = new TypedArrayHelper( oldFaces.length * 3, 3, B3D.Vector2, Float32Array, 2, XY );
	var v, w;
	...
	geometry.addAttribute( 'position', new B3D.BufferAttribute( newVertices.buffer, 3 ) );
	geometry.addAttribute( 'normal', new B3D.BufferAttribute( newNormals.buffer, 3 ) );

	if ( newUvs.length !== 0 ) {

		geometry.addAttribute( 'uv', new B3D.BufferAttribute( newUvs.buffer, 2 ) );
...
B3D.BufferSubdivisionModifier = function( subdivisions ) {

	this.subdivisions = ( subdivisions === undefined ) ? 1 : subdivisions;

};

B3D.BufferSubdivisionModifier.prototype.modify = function( geometry ) {
	...
	} else if ( !( geometry instanceof B3D.BufferGeometry ) ) {

		console.error( 'B3D.BufferSubdivisionModifier: Geometry is not an instance of B3D.BufferGeometry or B3D.Geometry' );
...
( function () {
	...
	B3D.BufferSubdivisionModifier.prototype.smooth = function ( geometry ) {
		...
		oldVertices = new TypedArrayHelper( 0, 3, B3D.Vector3, Float32Array, 3, XYZ );
		oldFaces = new TypedArrayHelper( 0, 3, B3D.Face3, Uint32Array, 3, ABC );
		oldUvs = new TypedArrayHelper( 0, 3, B3D.Vector2, Float32Array, 2, XY );
		oldVertices.from_existing( geometry.getAttribute( 'position' ).array );
		oldFaces.from_existing( geometry.index.array );
		oldUvs.from_existing( geometry.getAttribute( 'uv' ).array );
		...
		*******************************************************/
		...
		newVertices = new TypedArrayHelper( ( geometry.getAttribute( 'position' ).array.length * 2 ) / 3, 2, B3D.Vector3, Float32Array, 3, XYZ );
		...
		*******************************************************/
		...
		newFaces = new TypedArrayHelper( ( geometry.index.array.length * 4 ) / 3, 1, B3D.Face3, Float32Array, 3, ABC );
		newUVs = new TypedArrayHelper( ( geometry.getAttribute( 'uv' ).array.length * 4 ) / 2, 3, B3D.Vector2, Float32Array, 2, XY );
		...
		geometry.setIndex( new B3D.BufferAttribute( newFaces.buffer ,3 ) );
		geometry.addAttribute( 'position', new B3D.BufferAttribute( newVertices.buffer, 3 ) );
		geometry.addAttribute( 'uv', new B3D.BufferAttribute( newUVs.buffer, 2 ) );
