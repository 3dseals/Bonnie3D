...
B3D.VRMLLoader = function ( manager ) {

	this.manager = ( manager !== undefined ) ? manager : B3D.DefaultLoadingManager;

};

B3D.VRMLLoader.prototype = {

	constructor: B3D.VRMLLoader,
	...
	parse: function ( data ) {
		...
		var textureLoader = new B3D.TextureLoader( this.manager );
			...
			*/
			...
			var parseNode = function ( data, parent ) {
						...
						if ( undefined == defines[ defineKey ] ) {
							...
							if ( /appearance/.exec( data ) && defineKey ) {
								...
								if ( undefined !== defines[ defineKey ].solid && defines[ defineKey ].solid === false ) {
									...
									parent.material.side = B3D.DoubleSide;
				...
				if('SpotLight' === data.nodeType){
						...
						parent.add( object );
						/*
						var lightHelper = new B3D.SpotLightHelper( object );
				...
				if ( 'Transform' === data.nodeType || 'Group' === data.nodeType ) {
					...
					if ( undefined !== data.rotation ) {
						...
						object.quaternion.setFromAxisAngle( new B3D.Vector3( r.x, r.y, r.z ), r.w );
					...
					var skyGeometry = new B3D.SphereGeometry( radius, segments, segments );
					var skyMaterial = new B3D.MeshBasicMaterial( { fog: false, side: B3D.BackSide } );

					if ( data.skyColor.length > 1 ) {

						paintFaces( skyGeometry, radius, data.skyAngle, data.skyColor, true );

						skyMaterial.vertexColors = B3D.VertexColors
					...
					scene.add( new B3D.Mesh( skyGeometry, skyMaterial ) );
					...
					if ( data.groundColor !== undefined ) {
						...
						var groundGeometry = new B3D.SphereGeometry( radius, segments, segments, 0, 2 * Math.PI, 0.5 * Math.PI, 1.5 * Math.PI );
						var groundMaterial = new B3D.MeshBasicMaterial( { fog: false, side: B3D.BackSide, vertexColors: B3D.VertexColors } );

						paintFaces( groundGeometry, radius, data.groundAngle, data.groundColor, false );

						scene.add( new B3D.Mesh( groundGeometry, groundMaterial ) );
					...
					if ( 'Box' === data.nodeType ) {
						...
						parent.geometry = new B3D.BoxGeometry( s.x, s.y, s.z );

					} else if ( 'Cylinder' === data.nodeType ) {

						parent.geometry = new B3D.CylinderGeometry( data.radius, data.radius, data.height );

					} else if ( 'Cone' === data.nodeType ) {

						parent.geometry = new B3D.CylinderGeometry( data.topRadius, data.bottomRadius, data.height );

					} else if ( 'Sphere' === data.nodeType ) {

						parent.geometry = new B3D.SphereGeometry( data.radius );
							...
							if ( 'Coordinate' === child.nodeType ) {
								...
								if ( child.points ) {
									...
									for ( var k = 0, l = child.points.length; k < l; k ++ ) {
										...
										vec = new B3D.Vector3( point.x, point.y, point.z );
