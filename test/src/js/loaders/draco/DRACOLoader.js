...
B3D.DRACOLoader = function(manager) {
    this.manager = (manager !== undefined) ? manager :
        B3D.DefaultLoadingManager;
    this.materials = null;
    this.verbosity = 0;
...
B3D.DRACOLoader.prototype = {

    constructor: B3D.DRACOLoader,

    load: function(url, onLoad, onProgress, onError) {
        const scope = this;
        const loader = new B3D.FileLoader(scope.manager);
    ...
    decodeDracoFile: function(rawBuffer, callback) {
      const scope = this;
      B3D.DRACOLoader.getDecoder(this.dracoDecoderType,
    ...
    decodeDracoFileInternal : function(rawBuffer, dracoDecoder, callback) {
      ...
      if (geometryType == dracoDecoder.TRIANGULAR_MESH) {
        ...
        const errorMsg = 'B3D.DRACOLoader: Unknown geometry type.'
    ...
    convertDracoGeometryTo3JS: function(dracoDecoder, wrapper, geometryType,
        ...
        if (posAttId == -1) {
          const errorMsg = 'B3D.DRACOLoader: No position attribute found.';
          console.error(errorMsg);
        ...
        // Structure for converting to B3DJS geometry later.
        ...
        const geometry = new B3D.BufferGeometry();
        if (geometryType == dracoDecoder.TRIANGULAR_MESH) {
          geometry.setIndex(new(geometryBuffer.indices.length > 65535 ?
                B3D.Uint32BufferAttribute : B3D.Uint16BufferAttribute)
              (geometryBuffer.indices, 1));
        }
        geometry.addAttribute('position',
            new B3D.Float32BufferAttribute(geometryBuffer.vertices, 3));
        geometry.addAttribute('color',
            new B3D.Float32BufferAttribute(geometryBuffer.colors, 3));
        if (normalAttId != -1) {
          geometry.addAttribute('normal',
              new B3D.Float32BufferAttribute(geometryBuffer.normals, 3));
        }
        if (texCoordAttId != -1) {
          geometry.addAttribute('uv',
              new B3D.Float32BufferAttribute(geometryBuffer.uvs, 2));
        }
    ...
    isVersionSupported: function(version, callback) {
        return B3D.DRACOLoader.getDecoder(this.dracoDecoderType,
