 ...
 * @class
 *
 * @param {B3D.DefaultLoadingManager} [manager] The loadingManager for the loader to use. Default is {@link B3D.DefaultLoadingManager}
 */
B3D.OBJLoader2 = (function () {

	var OBJLOADER2_VERSION = '1.2.1';

	function OBJLoader2( manager ) {
		console.log( "Using B3D.OBJLoader2 version: " + OBJLOADER2_VERSION );
		this.manager = Validator.verifyInput( manager, B3D.DefaultLoadingManager );
	 ...
	 * @memberOf B3D.OBJLoader2
	 *
	 * @param {B3D.Object3D} sceneGraphBaseNode Scenegraph object where meshes will be attached
	 */
	...
	/**
	 * Set materials loaded by MTLLoader or any other supplier of an Array of {@link B3D.Material}.
	 * @memberOf B3D.OBJLoader2
	 *
	 * @param {B3D.Material[]} materials  Array of {@link B3D.Material} from MTLLoader
	 */
	...
	/**
	 * Constants used by B3D.OBJLoader2
	 */
	var Consts = {
		CODE_LF: 10,
		CODE_CR: 13,
	 ...
	 * It is defined publicly for inclusion in web worker based OBJ loader ({@link B3D.OBJLoader2.WWOBJLoader2})
	 */
	 ...
	 * @param {number} smoothingGroup Normalized smoothingGroup (0: B3D.FlatShading, 1: B3D.SmoothShading)
	 */
	...
	/**
	 * MeshCreator is used to transform RawObjectDescriptions to B3D.Mesh
	 *
	 * @class
	 */
	var MeshCreator = (function () {
		...
		MeshCreator.prototype.setSceneGraphBaseNode = function ( sceneGraphBaseNode ) {
			...
			this.sceneGraphBaseNode = Validator.verifyInput( this.sceneGraphBaseNode, new B3D.Group() );
		};
		...
		MeshCreator.prototype.buildMesh = function ( rawObjectDescriptions, inputObjectCount, absoluteVertexCount, absoluteNormalCount, absoluteUvCount ) {
			...
			var vertexBA = new B3D.BufferAttribute( new Float32Array( absoluteVertexCount ), 3 );
			bufferGeometry.addAttribute( 'position', vertexBA );

			var normalBA;
			if ( absoluteNormalCount > 0 ) {

				normalBA = new B3D.BufferAttribute( new Float32Array( absoluteNormalCount ), 3 );
			...
			if ( absoluteUvCount > 0 ) {
				...
				uvBA = new B3D.BufferAttribute( new Float32Array( absoluteUvCount ), 2 );
			...
			for ( var oodIndex in rawObjectDescriptions ) {
				...
				if ( ! material ) {
					...
					if ( ! material ) {
						...
						material = new B3D.MeshStandardMaterial( { color: 0xDCF1FF} );
