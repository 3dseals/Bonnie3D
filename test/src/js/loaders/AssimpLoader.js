...
( function () {
	...
	Virtulous.KeyFrame = function ( time, matrix ) {
		...
		this.quaternion = new B3D.Quaternion();
		this.scale = new B3D.Vector3( 1, 1, 1 );
	...
	Virtulous.KeyFrame.tempAniPos = new B3D.Vector3();
	Virtulous.KeyFrame.tempAniQuat = new B3D.Quaternion();
	Virtulous.KeyFrame.tempAniScale = new B3D.Vector3( 1, 1, 1 );
	Virtulous.KeyFrame.tempAniMatrix = new B3D.Matrix4();
	...
	function aiMesh() {
		...
		this.hookupSkeletons = function ( scene, threeScene ) {
			...
			var threeSkeletonRoot = skeletonRoot.toB3D( scene );
			...
			for ( var i = 0; i < this.mBones.length; i ++ ) {
				...
				if ( bone ) {
					...
					offsetMatrix.push( this.mBones[ i ].mOffsetMatrix.toB3D() );
			...
			var skeleton = new B3D.Skeleton( allBones, offsetMatrix );

			this.threeNode.bind( skeleton, new B3D.Matrix4() );
			this.threeNode.material.skinning = true;

		};

		this.toB3D = function ( scene ) {

			if ( this.threeNode ) return this.threeNode;
			var geometry = new B3D.BufferGeometry();
			var mat;
			if ( scene.mMaterials[ this.mMaterialIndex ] )
				mat = scene.mMaterials[ this.mMaterialIndex ].toB3D( scene );
			else
				mat = new B3D.MeshLambertMaterial();
			geometry.setIndex( new B3D.BufferAttribute( new Uint32Array( this.mIndexArray ), 1 ) );
			geometry.addAttribute( 'position', new B3D.BufferAttribute( this.mVertexBuffer, 3 ) );
			if ( this.mNormalBuffer && this.mNormalBuffer.length > 0 )
				geometry.addAttribute( 'normal', new B3D.BufferAttribute( this.mNormalBuffer, 3 ) );
			if ( this.mColorBuffer && this.mColorBuffer.length > 0 )
				geometry.addAttribute( 'color', new B3D.BufferAttribute( this.mColorBuffer, 4 ) );
			if ( this.mTexCoordsBuffers[ 0 ] && this.mTexCoordsBuffers[ 0 ].length > 0 )
				geometry.addAttribute( 'uv', new B3D.BufferAttribute( new Float32Array( this.mTexCoordsBuffers[ 0 ] ), 2 ) );
			if ( this.mTexCoordsBuffers[ 1 ] && this.mTexCoordsBuffers[ 1 ].length > 0 )
				geometry.addAttribute( 'uv1', new B3D.BufferAttribute( new Float32Array( this.mTexCoordsBuffers[ 1 ] ), 2 ) );
			if ( this.mTangentBuffer && this.mTangentBuffer.length > 0 )
				geometry.addAttribute( 'tangents', new B3D.BufferAttribute( this.mTangentBuffer, 3 ) );
			if ( this.mBitangentBuffer && this.mBitangentBuffer.length > 0 )
				geometry.addAttribute( 'bitangents', new B3D.BufferAttribute( this.mBitangentBuffer, 3 ) );
			if ( this.mBones.length > 0 ) {
				...
				geometry.addAttribute( 'skinWeight', new B3D.BufferAttribute( new Float32Array( _weights ), BONESPERVERT ) );
				geometry.addAttribute( 'skinIndex', new B3D.BufferAttribute( new Float32Array( _bones ), BONESPERVERT ) );
			...
			if ( this.mBones.length == 0 )
				mesh = new B3D.Mesh( geometry, mat );

			if ( this.mBones.length > 0 )
				mesh = new B3D.SkinnedMesh( geometry, mat );
	...
	function aiVector3D() {
		...
		this.toB3D = function () {
			...
			return new B3D.Vector3( this.x, this.y, this.z );
	...
	function aiVector2D() {
		...
		this.toB3D = function () {
			...
			return new B3D.Vector2( this.x, this.y );
	...
	function aiVector4D() {
		...
		this.toB3D = function () {
			...
			return new B3D.Vector4( this.w, this.x, this.y, this.z );
	...
	function aiColor4D() {
		...
		this.toB3D = function () {
			...
			return new B3D.Color( this.r, this.g, this.b, this.a );
	...
	function aiColor3D() {
		...
		this.toB3D = function () {
			...
			return new B3D.Color( this.r, this.g, this.b, 1 );
	...
	function aiQuaternion() {
		...
		this.toB3D = function () {
			...
			return new B3D.Quaternion( this.x, this.y, this.z, this.w );
	...
	function aiNode() {
		...
		this.toB3D = function ( scene ) {
			...
			if ( this.threeNode ) return this.threeNode;
			var o = new B3D.Object3D();
			o.name = this.mName;
			o.matrix = this.mTransformation.toB3D();

			for ( var i = 0; i < this.mChildren.length; i ++ ) {

				o.add( this.mChildren[ i ].toB3D( scene ) );

			}

			for ( var i = 0; i < this.mMeshes.length; i ++ ) {

				o.add( scene.mMeshes[ this.mMeshes[ i ] ].toB3D( scene ) );
	...
	function aiMaterialProperty() {
		...
		this.dataAsMap = function ( scene ) {
			...
			return new B3D.TextureLoader().load( baseURL + path );
	...
	function aiMaterial() {
		...
		this.toB3D = function ( scene ) {
			...
			var mat = new B3D.MeshPhongMaterial();
	...
	function sampleTrack( keys, time, lne, lerp ) {
		...
		if ( keys.length == 1 ) return keys[ 0 ].mValue.toB3D();
		...
		if ( key && nextKey ) {
			...
			return lerp( key.mValue.toB3D(), nextKey.mValue.toB3D(), l );
	...
	function aiNodeAnim() {
		...
		this.toB3D = function ( o, tps ) {
			...
			track.target = o.findNode( this.mNodeName ).toB3D();
	...
	function aiAnimation() {
		...
		this.toB3D = function ( root ) {
			...
			for ( var i in this.mChannels ) {
				...
				var tracks = this.mChannels[ i ].toB3D( root );
	...
	function aiScene() {
		...
		this.toB3D = function () {
			...
			var o = this.mRootNode.toB3D( this );
			...
			if ( this.mAnimations.length > 0 ) {
				...
				var a = this.mAnimations[ 0 ].toB3D( this );
