...
B3D.XLoader = function (manager, Texloader, _zflg) {

	this.manager = manager !== undefined ? manager : new B3D.DefaultLoadingManager();
	this.Texloader = Texloader !== undefined ? Texloader : new B3D.TextureLoader();
	this.zflg = _zflg === undefined ? false : _zflg;
	this.url = "";
	this.baseDir = "";
	this.nowReadMode = B3D.XLoader.XfileLoadMode.none;
	...
	this.putMatLength = 0;
	this.nowMat = null;
	this.BoneInf = new B3D.XLoader.XboneInf();
	this.tmpUvArray = [];
	this.normalVectors = [];
	this.facesNormal = [];
	this.nowFrameName = "";
...
B3D.XLoader.prototype = {

	constructor: B3D.XLoader,
	...
	parseASCII: function () {
		...
		scope.loadingXdata = new B3D.XLoader.Xdata();
	...
	readMeshSection: function (_baseOffset) {
		...
		var scope = this;
		scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Geometry = new B3D.Geometry();
	...
	readMaterial: function (_dataLine) {
		...
		var scope = this;
		scope.nowMat = new B3D.MeshPhongMaterial({ color: Math.random() * 0xffffff });
		if (scope.zflg) {

			scope.nowMat.side = B3D.BackSide;

		} else {

			scope.nowMat.side = B3D.FrontSide;
	...
	readSkinWeights: function (_data) {
		...
		var scope = this;
		scope.BoneInf = new B3D.XLoader.XboneInf();
		...
		scope.BoneInf.initMatrix = new B3D.Matrix4();
		scope.ParseMatrixData(scope.BoneInf.initMatrix, m_data);
		scope.BoneInf.OffsetMatrix = new B3D.Matrix4();
	...
	SectionRead: function () {
		...
		if (find3 > find2) {
			...
			if (line.indexOf("Frame ") > - 1) {
				...
				scope.nowReadMode = B3D.XLoader.XfileLoadMode.Mesh;
				return true;

			} else if (line.indexOf("MeshMaterialList ") > - 1) {

				scope.readMeshMaterialSet(find + 1);
				scope.nowReadMode = B3D.XLoader.XfileLoadMode.Mat_Set;
				return true;
				...
				scope.nowReadMode = B3D.XLoader.XfileLoadMode.Mat_detail;
				return true;

			} else if (line.indexOf("AnimationSet ") > - 1) {

				scope.readandCreateAnimationSet(line);
				scope.nowReadMode = B3D.XLoader.XfileLoadMode.Anim_init;
				...
				scope.nowReadMode = B3D.XLoader.XfileLoadMode.Anim_Reading;
			...
			if (line.indexOf("template ") > - 1) {
				...
				scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].FrameTransformMatrix = new B3D.Matrix4();
				scope.ParseMatrixData(scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].FrameTransformMatrix, data);
				scope.nowReadMode = B3D.XLoader.XfileLoadMode.Element;
				return false;
				...
				if (section[1].length > 0) {
					...
					scope.nowMat.normalScale = new B3D.Vector2(2, 2);
	...
	endElement: function (line) {
		...
		var scope = this;
		if (scope.nowReadMode == B3D.XLoader.XfileLoadMode.Mesh) {

			scope.nowReadMode = B3D.XLoader.XfileLoadMode.Element;

		} else if (scope.nowReadMode == B3D.XLoader.XfileLoadMode.Mat_Set) {

			scope.nowReadMode = B3D.XLoader.XfileLoadMode.Mesh;

		} else if (scope.nowReadMode == B3D.XLoader.XfileLoadMode.Mat_detail) {

			scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Materials.push(scope.nowMat);
			scope.nowReadMode = B3D.XLoader.XfileLoadMode.Mat_Set;

		} else if (scope.nowReadMode == B3D.XLoader.XfileLoadMode.Anim_Reading) {

			scope.nowReadMode = B3D.XLoader.XfileLoadMode.Anim_init;

		} else if (scope.nowReadMode < B3D.XLoader.XfileLoadMode.Anim_init && scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].FrameStartLv === scope.elementLv && scope.nowReadMode > B3D.XLoader.XfileLoadMode.none) {
		...
		} else if (scope.nowReadMode == B3D.XLoader.XfileLoadMode.Anim_init) {
	...
	beginFrame: function (line) {
		...
		scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName] = new B3D.XLoader.XFrameInfo();
	...
	beginReadMesh: function (line) {
		...
		scope.nowReadMode = B3D.XLoader.XfileLoadMode.Vartex_init;
		Bones = [];
		scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Materials = [];
	...
	readVertexCount: function (line) {
		...
		var scope = this;
		scope.nowReadMode = B3D.XLoader.XfileLoadMode.Vartex_Read;
	...
	readVertex: function (line) {
		...
		scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Geometry.vertices.push(new B3D.Vector3(parseFloat(data[0]), parseFloat(data[1]), parseFloat(data[2])));
		scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Geometry.skinIndices.push(new B3D.Vector4(0, 0, 0, 0));
		scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Geometry.skinWeights.push(new B3D.Vector4(1, 0, 0, 0));
	...
	readIndexLength: function (line) {
		...
		var scope = this;
		scope.nowReadMode = B3D.XLoader.XfileLoadMode.index_Read;
		scope.tgtLength = parseInt(line.substr(0, line.length - 1), 10);
		scope.nowReaded = 0;
	...
	readVertexIndex: function (line) {
		...
		if (scope.zflg) {
			...
			scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Geometry.faces.push(new B3D.Face3(parseInt(data[2], 10), parseInt(data[1], 10), parseInt(data[0], 10), new B3D.Vector3(1, 1, 1).normalize()));

		} else {

			scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Geometry.faces.push(new B3D.Face3(parseInt(data[0], 10), parseInt(data[1], 10), parseInt(data[2], 10), new B3D.Vector3(1, 1, 1).normalize()));
	...
	beginMeshNormal: function (line) {
		...
		var scope = this;
		scope.nowReadMode = B3D.XLoader.XfileLoadMode.Normal_V_init;
		scope.normalVectors = [];
		scope.facesNormal = [];
	...
	readMeshNormalCount: function (line) {
		...
		var scope = this;
		scope.nowReadMode = B3D.XLoader.XfileLoadMode.Normal_V_Read;
		scope.tgtLength = parseInt(line.substr(0, line.length - 1), 10);
		scope.nowReaded = 0;
	...
	readMeshNormalVertex: function (line) {
		...
		if (scope.nowReaded >= scope.tgtLength) {
			...
			scope.nowReadMode = B3D.XLoader.XfileLoadMode.Normal_I_init;
			return true;
	...
	readMeshNormalIndexCount: function (line) {
		...
		var scope = this;
		scope.nowReadMode = B3D.XLoader.XfileLoadMode.Normal_I_Read;
		scope.tgtLength = parseInt(line.substr(0, line.length - 1), 10);
		scope.nowReaded = 0;
	...
	readMeshNormalIndex: function (line) {
		...
		var v1 = new B3D.Vector3(scope.normalVectors[nowID][0], scope.normalVectors[nowID][1], scope.normalVectors[nowID][2]);
		nowID = parseInt(data[1], 10);
		var v2 = new B3D.Vector3(scope.normalVectors[nowID][0], scope.normalVectors[nowID][1], scope.normalVectors[nowID][2]);
		nowID = parseInt(data[2], 10);
		var v3 = new B3D.Vector3(scope.normalVectors[nowID][0], scope.normalVectors[nowID][1], scope.normalVectors[nowID][2]);
		if (scope.zflg) {
	...
	readUvInit: function (line) {
		...
		var scope = this;
		scope.nowReadMode = B3D.XLoader.XfileLoadMode.Uv_Read;
		scope.tgtLength = parseInt(line.substr(0, line.length - 1), 10);
		scope.nowReaded = 0;
		scope.tmpUvArray = [];
		scope.loadingXdata.FrameInfo_Raw[scope.nowFrameName].Geometry.faceVertexUvs[0] = [];
	...
	readUv: function (line) {
		...
		if (scope.IsUvYReverse) {
			...
			scope.tmpUvArray.push(new B3D.Vector2(parseFloat(data[0]), 1 - parseFloat(data[1])));

		} else {

			scope.tmpUvArray.push(new B3D.Vector2(parseFloat(data[0]), parseFloat(data[1])));
	...
	readMatrixSetLength: function (line) {
		...
		var scope = this;
		scope.nowReadMode = B3D.XLoader.XfileLoadMode.Mat_Face_Set;
		scope.tgtLength = parseInt(line.substr(0, line.length - 1), 10);
		scope.nowReaded = 0;
	...
	readandSetMaterial: function (line) {
		...
		if (line.indexOf("TextureFilename") > - 1) {
			...
			scope.nowReadMode = B3D.XLoader.XfileLoadMode.Mat_Set_Texture;

		} else if (line.indexOf("BumpMapFilename") > - 1) {

			scope.nowReadMode = B3D.XLoader.XfileLoadMode.Mat_Set_BumpTex;
			scope.nowMat.bumpScale = 0.05;

		} else if (line.indexOf("NormalMapFilename") > - 1) {

			scope.nowReadMode = B3D.XLoader.XfileLoadMode.Mat_Set_NormalTex;
			scope.nowMat.normalScale = new B3D.Vector2(2, 2);

		} else if (line.indexOf("EmissiveMapFilename") > - 1) {

			scope.nowReadMode = B3D.XLoader.XfileLoadMode.Mat_Set_EmissiveTex;

		} else if (line.indexOf("LightMapFilename") > - 1) {

			scope.nowReadMode = B3D.XLoader.XfileLoadMode.Mat_Set_LightTex;
	...
	readBoneInit: function (line) {
		...
		var scope = this;
		scope.nowReadMode = B3D.XLoader.XfileLoadMode.Weit_init;
	...
	readBoneVertexLength: function (line) {
		...
		var scope = this;
		scope.nowReadMode = B3D.XLoader.XfileLoadMode.Weit_Read_Index;
		scope.tgtLength = parseInt(line.substr(0, line.length - 1), 10);
		scope.nowReaded = 0;
	...
	readandSetBoneVertex: function (line) {
		...
		if (scope.nowReaded >= scope.tgtLength || line.indexOf(";") > - 1) {
			...
			scope.nowReadMode = B3D.XLoader.XfileLoadMode.Weit_Read_Value;
			scope.nowReaded = 0;
	...
	readandSetBoneWeightValue: function (line) {
		...
		if (scope.nowReaded >= scope.tgtLength || line.indexOf(";") > - 1) {
			...
			scope.nowReadMode = B3D.XLoader.XfileLoadMode.Weit_Read_Matrx;
	...
	readAndCreateAnimation: function (line) {
		...
		scope.loadingXdata.AnimationSetInfo[scope.nowAnimationSetName][scope.nowFrameName] = new B3D.XLoader.XAnimationInfo();
	...
	readAnimationKeyFrameValue: function (line) {
		...
		if (!frameFound) {
			...
			scope.keyInfo = new B3D.XLoader.XKeyFrameInfo();
			scope.keyInfo.matrix = new B3D.Matrix4();
		...
		switch (scope.nowAnimationKeyType) {
			...
			case 0:
				tmpM.makeRotationFromQuaternion(new B3D.Quaternion(parseFloat(data2[0]), parseFloat(data2[1]), parseFloat(data2[2]), parseFloat(data2[3])));
	...
	MakeOutputGeometry: function (nowFrameName, _zflg) {
		...
		if (scope.loadingXdata.FrameInfo_Raw[nowFrameName].Geometry != null) {
			...
			var mesh = null;
			var bufferGeometry = new B3D.BufferGeometry();
			if (putBones.length > 0) {
				...
				mesh = new B3D.SkinnedMesh(bufferGeometry.fromGeometry(scope.loadingXdata.FrameInfo_Raw[nowFrameName].Geometry), scope.loadingXdata.FrameInfo_Raw[nowFrameName].Materials);
				var skeleton = new B3D.Skeleton(putBones /*, BoneInverse*/);
				mesh.add(putBones[0]);
				mesh.bind(skeleton);

			} else {

				mesh = new B3D.Mesh(bufferGeometry.fromGeometry(scope.loadingXdata.FrameInfo_Raw[nowFrameName].Geometry), scope.loadingXdata.FrameInfo_Raw[nowFrameName].Materials);
	...
	animationFinalize_step: function () {
		...
		if (tgtModel != null) {
			...
			scope.loadingXdata.XAnimationObj[i] = new B3D.XLoader.XAnimationObj();
...
B3D.XLoader.XAnimationObj = function () {
...
B3D.XLoader.XAnimationObj.prototype = {

	constructor: B3D.XLoader.XAnimationObj,
	...
	makeBonekeys: function (XAnimationInfo, bone, parent) {
		...
		for (var i = 0; i < XAnimationInfo.keyFrames.length; i++) {
			...
			keyframe.pos = new B3D.Vector3().setFromMatrixPosition(keyframe.matrix);
			keyframe.rot = new B3D.Quaternion().setFromRotationMatrix(keyframe.matrix);
			keyframe.scl = new B3D.Vector3().setFromMatrixScale(keyframe.matrix);
...
B3D.XLoader.XAnimationInfo = function () {
...
B3D.XLoader.XFrameInfo = function () {
...
B3D.XLoader.XKeyFrameInfo = function () {
