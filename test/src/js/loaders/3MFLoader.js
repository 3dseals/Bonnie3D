B3D.ThreeMFLoader = function ( manager ) {
...
B3D.ThreeMFLoader.prototype = {

	constructor: B3D.ThreeMFLoader,
	...
	parse: function ( data ) {
		...
		function buildMesh( meshData, data3mf ) {
			...
			geometry.setIndex( new B3D.BufferAttribute( meshData[ 'triangles' ], 1 ) );
			geometry.addAttribute( 'position', new B3D.BufferAttribute( meshData[ 'vertices' ], 3 ) );

			if ( meshData[ 'colors' ] ) {

				geometry.addAttribute( 'color', new B3D.BufferAttribute( meshData[ 'colors' ], 3 ) );
			...
			if ( meshData[ 'colors' ] && 0 < meshData[ 'colors' ].length ) {
				...
				materialOpts[ 'vertexColors' ] = B3D.VertexColors;
			...
			var material = new B3D.MeshPhongMaterial( materialOpts );
			return new B3D.Mesh( geometry, material );
