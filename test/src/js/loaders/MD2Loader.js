...
B3D.MD2Loader = function ( manager ) {

	this.manager = ( manager !== undefined ) ? manager : B3D.DefaultLoadingManager;

};

B3D.MD2Loader.prototype = {

	constructor: B3D.MD2Loader,
	...
	parse: ( function () {
		...
		return function ( buffer ) {
			...
			for ( var i = 0, l = header.num_st; i < l; i ++ ) {
				...
				uvs.push( new B3D.Vector2( u / header.skinwidth, 1 - ( v / header.skinheight ) ) );
			...
			for ( var i = 0, l = header.num_tris; i < l; i ++ ) {
				...
				geometry.faces.push( new B3D.Face3( a, b, c ) );
			...
			var translation = new B3D.Vector3();
			...
			for ( var i = 0, l = header.num_frames; i < l; i ++ ) {
				...
				for ( var j = 0; j < header.num_vertices; j ++ ) {
					...
					var normal = new B3D.Vector3( n[ 0 ], n[ 2 ], n[ 1 ] );
			...
			geometry.animations = B3D.AnimationClip.CreateClipsFromMorphTargetSequences( geometry.morphTargets, 10 );
