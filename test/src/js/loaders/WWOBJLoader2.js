...
if ( B3D.OBJLoader2 === undefined ) { B3D.OBJLoader2 = {} }
 ...
 * @class
 */
B3D.OBJLoader2.WWOBJLoader2 = (function () {

	var WWOBJLOADER2_VERSION = '1.2.1';

	var Validator = B3D.OBJLoader2.prototype._getValidator();
	...
	WWOBJLoader2.prototype._init = function () {
		console.log( "Using B3D.OBJLoader2.WWOBJLoader2 version: " + WWOBJLOADER2_VERSION );
		...
		this.manager = B3D.DefaultLoadingManager;
		this.fileLoader = new B3D.FileLoader( this.manager );
		this.mtlLoader = null;
	 ...
	 * @memberOf B3D.OBJLoader2.WWOBJLoader2
	 *
	 ...
	 * Use {@link B3D.OBJLoader2.WWOBJLoader2.LoadedMeshUserOverride} for alteration instructions (geometry, material or disregard mesh).
	 * @memberOf B3D.OBJLoader2.WWOBJLoader2
	 *
	...
	WWOBJLoader2.prototype._validate = function () {
		...
		this.fileLoader = Validator.verifyInput( this.fileLoader, new B3D.FileLoader( this.manager ) );
		this.mtlLoader = Validator.verifyInput( this.mtlLoader, new B3D.MTLLoader() );
		...
		this.materials = [];
		var defaultMaterial = new B3D.MeshStandardMaterial( { color: 0xDCF1FF } );
	 ...
	 * @memberOf B3D.OBJLoader2.WWOBJLoader2
	 *
	 * @param {Object} params Either {@link B3D.OBJLoader2.WWOBJLoader2.PrepDataArrayBuffer} or {@link B3D.OBJLoader2.WWOBJLoader2.PrepDataFile}
	 */
	...
	WWOBJLoader2.prototype._receiveWorkerMessage = function ( event ) {
		...
		switch ( payload.cmd ) {
			case 'objData':
				...
				bufferGeometry.addAttribute( 'position', new B3D.BufferAttribute( new Float32Array( payload.vertices ), 3 ) );
				if ( Validator.isValid( payload.normals ) ) {

					bufferGeometry.addAttribute( 'normal', new B3D.BufferAttribute( new Float32Array( payload.normals ), 3 ) );
				...
				if ( Validator.isValid( payload.uvs ) ) {
					...
					bufferGeometry.addAttribute( 'uv', new B3D.BufferAttribute( new Float32Array( payload.uvs ), 2 ) );
				...
				for ( key in materialDescriptions ) {
					...
					if ( materialDescription.default ) {
						...
						if ( ! materialClone ) {
							...
							materialClone.shading = B3D.FlatShading;
				...
				if ( !disregardMesh ) {
					...
					var mesh = new B3D.Mesh( bufferGeometry, material );
					mesh.name = meshName;
	...
	WWOBJLoader2.prototype._buildWebWorkerCode = function ( existingWorkerCode ) {
		...
		if ( ! Validator.isValid( this.workerCode ) ) {
			...
			var wwMeshCreatorDef = (function () {
				...
				/**
				 * RawObjectDescriptions are transformed to B3D.Mesh.
			...
			this.workerCode += B3D.OBJLoader2.prototype._buildWebWorkerCode( buildObject, buildSingelton );
...
/**
 * Instruction to configure {@link B3D.OBJLoader2.WWOBJLoader2}.prepareRun to load OBJ from given ArrayBuffer and MTL from given String.
 *
 ...
 * @constructor
 */
B3D.OBJLoader2.WWOBJLoader2.PrepDataArrayBuffer = function ( modelName, objAsArrayBuffer, pathTexture, mtlAsString ) {
	...
	return {
		...
		/**
		 * {@link B3D.Object3D} where meshes will be attached.
		 * @memberOf B3D.OBJLoader2.WWOBJLoader2.PrepDataArrayBuffer
		 *
		 * @param {B3D.Object3D} sceneGraphBaseNode Scene graph object
		 */
		...
		/**
		 * Returns all callbacks as {@link B3D.OBJLoader2.WWOBJLoader2.PrepDataCallbacks}
		 * @memberOf B3D.OBJLoader2.WWOBJLoader2.PrepDataArrayBuffer
		 *
		 * @returns {B3D.OBJLoader2.WWOBJLoader2.PrepDataCallbacks}
		 */
		...
		callbacks: new B3D.OBJLoader2.WWOBJLoader2.PrepDataCallbacks()
	};
};

/**
 * Instruction to configure {@link B3D.OBJLoader2.WWOBJLoader2}.prepareRun to load OBJ and MTL from files.
 *
 * @param {string} modelName Overall name of the model
 ...
 * @constructor
 */
B3D.OBJLoader2.WWOBJLoader2.PrepDataFile = function ( modelName, pathObj, fileObj, pathTexture, fileMtl ) {
	...
	return {
		...
		/**
		 * {@link B3D.Object3D} where meshes will be attached.
		 * @memberOf B3D.OBJLoader2.WWOBJLoader2.PrepDataFile
		 *
		 * @param {B3D.Object3D} sceneGraphBaseNode Scene graph object
		 */
		setSceneGraphBaseNode: function ( sceneGraphBaseNode ) {
...
/**
 * Callbacks utilized by functions working with {@link B3D.OBJLoader2.WWOBJLoader2.PrepDataArrayBuffer} or {@link B3D.OBJLoader2.WWOBJLoader2.PrepDataFile}
 *
 * @returns {{registerCallbackProgress: B3D.OBJLoader2.WWOBJLoader2.PrepDataCallbacks.registerCallbackProgress, registerCallbackCompletedLoading: B3D.OBJLoader2.WWOBJLoader2.PrepDataCallbacks.registerCallbackCompletedLoading, registerCallbackMaterialsLoaded: B3D.OBJLoader2.WWOBJLoader2.PrepDataCallbacks.registerCallbackMaterialsLoaded, registerCallbackMeshLoaded: B3D.OBJLoader2.WWOBJLoader2.PrepDataCallbacks.registerCallbackMeshLoaded, registerCallbackErrorWhileLoading: B3D.OBJLoader2.WWOBJLoader2.PrepDataCallbacks.registerCallbackErrorWhileLoading, progress: null, completedLoading: null, errorWhileLoading: null, materialsLoaded: null, meshLoaded: null}}
 * @constructor
 */
B3D.OBJLoader2.WWOBJLoader2.PrepDataCallbacks = function () {
	...
	return {
		/**
		 * Register callback function that is invoked by internal function "_announceProgress" to print feedback.
		 * @memberOf B3D.OBJLoader2.WWOBJLoader2.PrepDataCallbacks
		 *
		 * @param {callback} callbackProgress Callback function for described functionality
		 */
...
/**
 * Object to return by {@link B3D.OBJLoader2.WWOBJLoader2}.callbacks.meshLoaded. Used to adjust bufferGeometry or material or prevent complete loading of mesh
 *
 * @param {boolean} disregardMesh=false Tell WWOBJLoader2 to completely disregard this mesh
 * @param {B3D.BufferGeometry} bufferGeometry The {@link B3D.BufferGeometry} to be used
 * @param {B3D.Material} material The {@link B3D.Material} to be used
 *
 * @returns {{ disregardMesh: boolean, replaceBufferGeometry: boolean, bufferGeometry: B3D.BufferGeometry, replaceMaterial: boolean, material: B3D.Material}}
 * @constructor
 */
B3D.OBJLoader2.WWOBJLoader2.LoadedMeshUserOverride = function ( disregardMesh, bufferGeometry, material ) {
 ...
 * @class
 */
B3D.OBJLoader2.WWOBJLoader2Director = (function () {
	...
	function WWOBJLoader2Director() {
		...
		this.workerDescription = {
			prototypeDef: B3D.OBJLoader2.WWOBJLoader2.prototype,
			globalCallbacks: {},
			webWorkers: [],
			codeBuffer: null
		};
	 ...
	 * @memberOf B3D.OBJLoader2.WWOBJLoader2Director
	 *
	 * @returns {number}
	 */
	 ...
	 * @memberOf B3D.OBJLoader2.WWOBJLoader2Director
	 *
	 * @param {B3D.OBJLoader2.WWOBJLoader2.PrepDataCallbacks} globalCallbacks  Register global callbacks used by all web workers
	 ...
	 * @memberOf B3D.OBJLoader2.WWOBJLoader2Director
	 *
	 * @param {Object} runParams Either {@link B3D.OBJLoader2.WWOBJLoader2.PrepDataArrayBuffer} or {@link B3D.OBJLoader2.WWOBJLoader2.PrepDataFile}
	 */
