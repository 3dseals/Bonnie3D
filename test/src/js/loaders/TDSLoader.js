...
B3D.TDSLoader = function ( manager ) {
...
B3D.TDSLoader.prototype = {
	...
	readMaterialEntry : function ( data ) {
		...
		while ( next !== 0 ) {
			...
			if ( next === MAT_NAME ) {
				...
				material.side = B3D.DoubleSide;
				this.debugMessage( '   DoubleSided' );

			} else if ( next === MAT_ADDITIVE ) {

				this.debugMessage( '   Additive Blending' );
				material.blending = B3D.AdditiveBlending;
		...
		while ( next !== 0 ) {
			...
			if ( next === POINT_ARRAY ) {
				...
				if ( useBufferGeometry )	{
					...
					geometry.addAttribute( 'position', new B3D.BufferAttribute( new Float32Array( vertices ), 3 ) );

				} else	{ //Geometry

					for ( var i = 0; i < points; i ++ )		{

						geometry.vertices.push( new B3D.Vector3( this.readFloat( data ), this.readFloat( data ), this.readFloat( data ) ) );
				...
				if ( useBufferGeometry )	{
					...
					geometry.addAttribute( 'uv', new B3D.BufferAttribute( new Float32Array( uvs ), 2 ) );
					...
					for ( var i = 0; i < texels; i ++ )		{
						...
						uvs.push( new B3D.Vector2( this.readFloat( data ), this.readFloat( data ) ) );
	...
	readFaceArray : function ( data, mesh ) {
		...
		for ( var i = 0; i < faces; ++ i ) {
			...
			mesh.geometry.faces.push( new B3D.Face3( this.readWord( data ), this.readWord( data ), this.readWord( data ) ) );
