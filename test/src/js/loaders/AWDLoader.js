...
( function () {
	...
	B3D.AWDLoader.prototype = {

		constructor: B3D.AWDLoader,
	...
	//        break;
		...
		parseMeshInstance: function ( len ) {
			...
			if ( meshLen  > 1 ) {
				...
				mesh = new B3D.Object3D();
				for ( i = 0; i < meshLen; i ++ ) {

					var sm = new B3D.Mesh( geometries[ i ] );
					meshes.push( sm );
					mesh.add( sm );
				...
				mesh = new B3D.Mesh( geometries[ 0 ] );
				meshes.push( mesh );
		...
		loadTexture: function( url ) {
			...
			var tex = new B3D.Texture();

			var loader = new B3D.ImageLoader( this.manager );
			...
			while ( subs_parsed < num_subs ) {
				...
				while ( this._ptr < sm_end ) {
					...
					if ( str_type === 1 ) {
						...
						attrib = new B3D.BufferAttribute( buffer, 3 );
					...
					// -----------------
						...
						attrib = new B3D.BufferAttribute( buffer, 1 );
					...
					// -------------------
						...
						attrib = new B3D.BufferAttribute( buffer, 2 );
