...
B3D.PDBLoader = function ( manager ) {

	this.manager = ( manager !== undefined ) ? manager : B3D.DefaultLoadingManager;

};

B3D.PDBLoader.prototype = {

	constructor: B3D.PDBLoader,
	...
	createModel: function ( json, callback ) {
		...
		var geometryAtoms = new B3D.BufferGeometry();
		var geometryBonds = new B3D.BufferGeometry();
		...
		geometryAtoms.addAttribute( 'position', new B3D.Float32BufferAttribute( verticesAtoms, 3 ) );
		geometryAtoms.addAttribute( 'color', new B3D.Float32BufferAttribute( colors, 3 ) );

		geometryBonds.addAttribute( 'position', new B3D.Float32BufferAttribute( verticesBonds, 3 ) );
