...
B3D.AMFLoader = function ( manager ) {

	this.manager = ( manager !== undefined ) ? manager : B3D.DefaultLoadingManager;

};

B3D.AMFLoader.prototype = {

	constructor: B3D.AMFLoader,
	...
	parse: function ( data ) {
		...
		function loadMaterials( node ) {
			...
			loadedMaterial = new B3D.MeshPhongMaterial( {
				shading: B3D.FlatShading,
				color: new B3D.Color( color.r, color.g, color.b ),
				name: matName
			} );
		...
		var sceneObject = new B3D.Group();
		var defaultMaterial = new B3D.MeshPhongMaterial( { color: 0xaaaaff, shading: B3D.FlatShading } );
		...
		for ( var id in amfObjects ) {
			...
			for ( var i = 0; i < meshes.length; i ++ ) {
				...
				var mesh = meshes[ i ];
				var vertices = new B3D.BufferAttribute( new Float32Array( mesh.vertices ), 3 );
				var normals = null;

				if ( mesh.normals.length ) {

					normals = new B3D.BufferAttribute( new Float32Array( mesh.normals ), 3 );
				...
				if ( mesh.color ) {
					...
					objDefaultMaterial.color = new B3D.Color( color.r, color.g, color.b );
				...
				for ( var j = 0; j < volumes.length; j ++ ) {
					...
					var newGeometry = new B3D.BufferGeometry();
					var indexes = new  Uint32Array( volume.triangles );
					var material = objDefaultMaterial;

					newGeometry.setIndex( new B3D.BufferAttribute( indexes, 1 ) );
					...
					newObject.add( new B3D.Mesh( newGeometry, material.clone() ) );
