...
B3D.MMDLoader = function ( manager ) {

	B3D.Loader.call( this );
...
B3D.MMDLoader.prototype = Object.create( B3D.Loader.prototype );
B3D.MMDLoader.prototype.constructor = B3D.MMDLoader;
 ...
 */
B3D.MMDLoader.prototype.defaultToonTextures = [
 ...
 */
B3D.MMDLoader.prototype.setTextureCrossOrigin = function ( value ) {

	this.textureCrossOrigin = value;

};

B3D.MMDLoader.prototype.load = function ( modelUrl, vmdUrls, callback, onProgress, onError ) {
...
B3D.MMDLoader.prototype.loadModel = function ( url, callback, onProgress, onError ) {
...
B3D.MMDLoader.prototype.createModel = function ( buffer, modelExtension, texturePath, onProgress, onError ) {

	return this.createMesh( this.parseModel( buffer, modelExtension ), texturePath, onProgress, onError );

};

B3D.MMDLoader.prototype.loadVmd = function ( url, callback, onProgress, onError ) {
...
B3D.MMDLoader.prototype.loadVmds = function ( urls, callback, onProgress, onError ) {
...
B3D.MMDLoader.prototype.loadAudio = function ( url, callback, onProgress, onError ) {

	var listener = new B3D.AudioListener();
	var audio = new B3D.Audio( listener );
	var loader = new B3D.AudioLoader( this.manager );
...
B3D.MMDLoader.prototype.loadVpd = function ( url, callback, onProgress, onError, params ) {
...
B3D.MMDLoader.prototype.parseModel = function ( buffer, modelExtension ) {
...
B3D.MMDLoader.prototype.parsePmd = function ( buffer ) {

	return this.parser.parsePmd( buffer, true );

};

B3D.MMDLoader.prototype.parsePmx = function ( buffer ) {

	return this.parser.parsePmx( buffer, true );

};

B3D.MMDLoader.prototype.parseVmd = function ( buffer ) {

	return this.parser.parseVmd( buffer, true );

};

B3D.MMDLoader.prototype.parseVpd = function ( text ) {

	return this.parser.parseVpd( text, true );

};

B3D.MMDLoader.prototype.mergeVmds = function ( vmds ) {

	return this.parser.mergeVmds( vmds );

};

B3D.MMDLoader.prototype.pourVmdIntoModel = function ( mesh, vmd, name ) {

	this.createAnimation( mesh, vmd, name );

};

B3D.MMDLoader.prototype.pourVmdIntoCamera = function ( camera, vmd, name ) {

	var helper = new B3D.MMDLoader.DataCreationHelper();
	...
	var initAnimation = function () {
		...
		var quaternion = new B3D.Quaternion();
		var euler = new B3D.Euler();
		var position = new B3D.Vector3();
		var center = new B3D.Vector3();
		...
		var createTrack = function ( node, type, times, values, interpolations ) {
			...
			return new B3D.MMDLoader[ type ]( node, times, values, interpolations );
		...
		var clip = new B3D.AnimationClip( name === undefined ? B3D.Math.generateUUID() : name, -1, tracks );

		if ( camera.center === undefined ) camera.center = new B3D.Vector3( 0, 0, 0 );
...
B3D.MMDLoader.prototype.extractExtension = function ( url ) {
...
B3D.MMDLoader.prototype.loadFile = function ( url, onLoad, onProgress, onError, responseType, mimeType ) {
...
B3D.MMDLoader.prototype.loadFileAsBuffer = function ( url, onLoad, onProgress, onError ) {

	this.loadFile( url, onLoad, onProgress, onError, 'arraybuffer' );

};

B3D.MMDLoader.prototype.loadFileAsText = function ( url, onLoad, onProgress, onError ) {

	this.loadFile( url, onLoad, onProgress, onError, 'text' );

};

B3D.MMDLoader.prototype.loadFileAsShiftJISText = function ( url, onLoad, onProgress, onError ) {

	this.loadFile( url, onLoad, onProgress, onError, 'text', 'text/plain; charset=shift_jis' );

};

B3D.MMDLoader.prototype.createMesh = function ( model, texturePath, onProgress, onError ) {
			...
			for ( var i = 0; i < model.metadata.ikCount; i++ ) {
				...
				for ( var j = 0; j < ik.links.length; j++ ) {
					...
					if ( model.bones[ link.index ].name.indexOf( 'ひざ' ) >= 0 ) {
						...
						link.limitation = new B3D.Vector3( 1.0, 0.0, 0.0 );
	...
	var initMorphs = function () {
		...
		for ( var i = 0; i < model.metadata.morphCount; i++ ) {
			...
			var attribute = new B3D.Float32BufferAttribute( model.metadata.vertexCount * 3, 3 );
	...
	var initMaterials = function () {
		...
		var textures = {};
		var textureLoader = new B3D.TextureLoader( scope.manager );
		var tgaLoader = new B3D.TGALoader( scope.manager );
		...
		function loadTexture ( filePath, params ) {
			...
			if ( params.defaultTexturePath === true ) {
				...
				try {
					...
					console.warn( 'B3D.MMDLoader: ' + filePath + ' seems like not right default texture path. Using toon00.bmp instead.' );
			...
			var loader = B3D.Loader.Handlers.get( fullPath );
			...
			var texture = loader.load( fullPath, function ( t ) {
				...
				t.flipY = false;
				t.wrapS = B3D.RepeatWrapping;
				t.wrapT = B3D.RepeatWrapping;
			...
			if ( params.sphericalReflectionMapping === true ) {
				...
				texture.mapping = B3D.SphericalReflectionMapping;
		...
		function getTexture( name, textures ) {
			...
			if ( textures[ name ] === undefined ) {
				...
				console.warn( 'B3D.MMDLoader: Undefined texture', name );
		...
		for ( var i = 0; i < model.metadata.materialCount; i++ ) {
			 ...
			 */
			params.color = new B3D.Color( m.diffuse[ 0 ], m.diffuse[ 1 ], m.diffuse[ 2 ] );
			params.opacity = m.diffuse[ 3 ];
			params.specular = new B3D.Color( m.specular[ 0 ], m.specular[ 1 ], m.specular[ 2 ] );
			params.shininess = m.shininess;

			if ( params.opacity === 1.0 ) {

				params.side = B3D.FrontSide;
				params.transparent = false;

			} else {

				params.side = B3D.DoubleSide;
					...
					for ( var j = 0; j < fileNames.length; j++ ) {
						...
						if ( n.indexOf( '.sph' ) >= 0 || n.indexOf( '.spa' ) >= 0 ) {
							...
							if ( n.indexOf( '.sph' ) >= 0 ) {
								...
								params.envMapType = B3D.MultiplyOperation;

							} else {

								params.envMapType = B3D.AddOperation;
			...
			params.emissive = new B3D.Color( m.ambient[ 0 ] * coef, m.ambient[ 1 ] * coef, m.ambient[ 2 ] * coef );
		...
		for ( var i = 0; i < materialParams.length; i++ ) {
			...
			var m = new B3D.MeshToonMaterial();
			...
			m.lights = true;
			m.side = ( model.metadata.format === 'pmx' && ( p2.flag & 0x1 ) === 1 ) ? B3D.DoubleSide : p.side;
			m.transparent = p.transparent;
			m.fog = true;

			m.blending = B3D.CustomBlending;
			m.blendSrc = B3D.SrcAlphaFactor;
			m.blendDst = B3D.OneMinusSrcAlphaFactor;
			m.blendSrcAlpha = B3D.SrcAlphaFactor;
			m.blendDstAlpha = B3D.DstAlphaFactor;
			...
			if ( model.metadata.format === 'pmd' ) {
				...
				m.outlineParameters = {
					...
					color: new B3D.Color( 0.0, 0.0, 0.0 ),
					alpha: 1.0
				};
				...
				m.outlineParameters = {
					...
					color: new B3D.Color( p2.edgeColor[ 0 ], p2.edgeColor[ 1 ], p2.edgeColor[ 2 ] ),
	...
	var initGeometry = function () {
		...
		geometry.addAttribute( 'position', new B3D.Float32BufferAttribute( buffer.vertices, 3 ) );
		geometry.addAttribute( 'normal', new B3D.Float32BufferAttribute( buffer.normals, 3 ) );
		geometry.addAttribute( 'uv', new B3D.Float32BufferAttribute( buffer.uvs, 2 ) );
		geometry.addAttribute( 'skinIndex', new B3D.Float32BufferAttribute( buffer.skinIndices, 4 ) );
		geometry.addAttribute( 'skinWeight', new B3D.Float32BufferAttribute( buffer.skinWeights, 4 ) );
	...
	var mesh = new B3D.SkinnedMesh( geometry, materials );
...
B3D.MMDLoader.prototype.createAnimation = function ( mesh, vmd, name ) {
	...
	var initMotionAnimations = function () {
		...
		for ( var i = 0; i < orderedMotions.length; i++ ) {
			...
			tracks.push( new B3D.MMDLoader.VectorKeyframeTrackEx( boneName + '.position', times, positions, pInterpolations ) );
			tracks.push( new B3D.MMDLoader.QuaternionKeyframeTrackEx( boneName + '.quaternion', times, rotations, rInterpolations ) );
	...
	var initMorphAnimations = function () {
		...
		for ( var i = 0; i < orderedMorphs.length; i++ ) {
			...
			tracks.push( new B3D.NumberKeyframeTrack( '.morphTargetInfluences[' + i + ']', times, values ) );

		}

		var clip = new B3D.AnimationClip( name === undefined ? B3D.Math.generateUUID() : name + 'Morph', -1, tracks );
...
B3D.MMDLoader.DataCreationHelper = function () {

};

B3D.MMDLoader.DataCreationHelper.prototype = {

	constructor: B3D.MMDLoader.DataCreationHelper,
 ...
 */
B3D.MMDLoader.VectorKeyframeTrackEx = function ( name, times, values, interpolationParameterArray ) {

	this.interpolationParameters = new Float32Array( interpolationParameterArray );

	B3D.VectorKeyframeTrack.call( this, name, times, values );

};

B3D.MMDLoader.VectorKeyframeTrackEx.prototype = Object.create( B3D.VectorKeyframeTrack.prototype );
B3D.MMDLoader.VectorKeyframeTrackEx.prototype.constructor = B3D.MMDLoader.VectorKeyframeTrackEx;
B3D.MMDLoader.VectorKeyframeTrackEx.prototype.TimeBufferType = Float64Array;

B3D.MMDLoader.VectorKeyframeTrackEx.prototype.InterpolantFactoryMethodCubicBezier = function( result ) {

	return new B3D.MMDLoader.CubicBezierInterpolation( this.times, this.values, this.getValueSize(), result, this.interpolationParameters );

};

B3D.MMDLoader.VectorKeyframeTrackEx.prototype.setInterpolation = function( interpolation ) {

	this.createInterpolant = this.InterpolantFactoryMethodCubicBezier;

};

B3D.MMDLoader.QuaternionKeyframeTrackEx = function ( name, times, values, interpolationParameterArray ) {

	this.interpolationParameters = new Float32Array( interpolationParameterArray );

	B3D.QuaternionKeyframeTrack.call( this, name, times, values );

};

B3D.MMDLoader.QuaternionKeyframeTrackEx.prototype = Object.create( B3D.QuaternionKeyframeTrack.prototype );
B3D.MMDLoader.QuaternionKeyframeTrackEx.prototype.constructor = B3D.MMDLoader.QuaternionKeyframeTrackEx;
B3D.MMDLoader.QuaternionKeyframeTrackEx.prototype.TimeBufferType = Float64Array;

B3D.MMDLoader.QuaternionKeyframeTrackEx.prototype.InterpolantFactoryMethodCubicBezier = function( result ) {

	return new B3D.MMDLoader.CubicBezierInterpolation( this.times, this.values, this.getValueSize(), result, this.interpolationParameters );

};

B3D.MMDLoader.QuaternionKeyframeTrackEx.prototype.setInterpolation = function( interpolation ) {

	this.createInterpolant = this.InterpolantFactoryMethodCubicBezier;

};

B3D.MMDLoader.NumberKeyframeTrackEx = function ( name, times, values, interpolationParameterArray ) {

	this.interpolationParameters = new Float32Array( interpolationParameterArray );

	B3D.NumberKeyframeTrack.call( this, name, times, values );

};

B3D.MMDLoader.NumberKeyframeTrackEx.prototype = Object.create( B3D.NumberKeyframeTrack.prototype );
B3D.MMDLoader.NumberKeyframeTrackEx.prototype.constructor = B3D.MMDLoader.NumberKeyframeTrackEx;
B3D.MMDLoader.NumberKeyframeTrackEx.prototype.TimeBufferType = Float64Array;

B3D.MMDLoader.NumberKeyframeTrackEx.prototype.InterpolantFactoryMethodCubicBezier = function( result ) {

	return new B3D.MMDLoader.CubicBezierInterpolation( this.times, this.values, this.getValueSize(), result, this.interpolationParameters );

};

B3D.MMDLoader.NumberKeyframeTrackEx.prototype.setInterpolation = function( interpolation ) {

	this.createInterpolant = this.InterpolantFactoryMethodCubicBezier;

};

B3D.MMDLoader.CubicBezierInterpolation = function ( parameterPositions, sampleValues, sampleSize, resultBuffer, params ) {

	B3D.Interpolant.call( this, parameterPositions, sampleValues, sampleSize, resultBuffer );

	this.params = params;

}

B3D.MMDLoader.CubicBezierInterpolation.prototype = Object.create( B3D.LinearInterpolant.prototype );
B3D.MMDLoader.CubicBezierInterpolation.prototype.constructor = B3D.MMDLoader.CubicBezierInterpolation;

B3D.MMDLoader.CubicBezierInterpolation.prototype.interpolate_ = function( i1, t0, t, t1 ) {
	...
	if ( stride === 4 ) {  // Quaternion
		...
		B3D.Quaternion.slerpFlat( result, 0, values, offset0, values, offset1, ratio );
...
B3D.MMDLoader.CubicBezierInterpolation.prototype._calculate = function( x1, x2, y1, y2, x ) {
...
B3D.MMDAudioManager = function ( audio, listener, p ) {
...
B3D.MMDAudioManager.prototype = {

	constructor: B3D.MMDAudioManager,
...
B3D.MMDGrantSolver = function ( mesh ) {

	this.mesh = mesh;

};

B3D.MMDGrantSolver.prototype = {

	constructor: B3D.MMDGrantSolver,

	update: function () {

		var q = new B3D.Quaternion();
...
B3D.MMDHelper.prototype = {

	constructor: B3D.MMDHelper,

	add: function ( mesh ) {

		if ( ! ( mesh instanceof B3D.SkinnedMesh ) ) {

			throw new Error( 'B3D.MMDHelper.add() accepts only B3D.SkinnedMesh instance.' );
	...
	setAudio: function ( audio, listener, params ) {
		...
		this.audioManager = new B3D.MMDAudioManager( audio, listener, params );
	...
	setPhysics: function ( mesh, params ) {
		...
		var physics = new B3D.MMDPhysics( mesh, params );
	...
	setAnimation: function ( mesh ) {
		...
		if ( mesh.geometry.animations !== undefined ) {
			...
			mesh.mixer = new B3D.AnimationMixer( mesh );
			...
			if ( foundAnimation ) {
				...
				mesh.ikSolver = new B3D.CCDIKSolver( mesh );

				if ( mesh.geometry.grants !== undefined ) {

					mesh.grantSolver = new B3D.MMDGrantSolver( mesh );
	...
	setCameraAnimation: function ( camera ) {
		...
		if ( camera.animations !== undefined ) {
			...
			camera.mixer = new B3D.AnimationMixer( camera );
	...
	poseAsVpd: function ( mesh, vpd, params ) {
		...
		var thV = new B3D.Vector3();
		var thQ = new B3D.Quaternion();
		...
		if ( params.preventIk !== true ) {
			...
			var solver = new B3D.CCDIKSolver( mesh );
		...
		if ( params.preventGrant !== true && mesh.geometry.grants !== undefined ) {
			...
			var solver = new B3D.MMDGrantSolver( mesh );
			solver.update();
	 ...
	 *       B3D.PropertyMixer.apply() seems to save values into buffer cache
