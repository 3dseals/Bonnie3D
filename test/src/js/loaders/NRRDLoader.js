B3D.NRRDLoader = function ( manager ) {
...
B3D.NRRDLoader.prototype = {

	constructor: B3D.NRRDLoader,
	...
	parse: function ( data ) {
		...
		function parseHeader( header ) {
			...
			for ( _i = 0, _len = lines.length; _i < _len; _i ++ ) {
				...
				if ( l.match( /NRRD\d+/ ) ) {
					...
					field = m[ 1 ].trim();
					data = m[ 2 ].trim();
					fn = B3D.NRRDLoader.prototype.fieldFunctions[ field ];
					if ( fn ) {
			...
			if ( ! headerObject.vectors ) {
				...
				headerObject.vectors = [ new B3D.Vector3( 1, 0, 0 ), new B3D.Vector3( 0, 1, 0 ), new B3D.Vector3( 0, 0, 1 ) ];
		...
		// spacing
		var spacingX = ( new B3D.Vector3( headerObject.vectors[ 0 ][ 0 ], headerObject.vectors[ 0 ][ 1 ],
		headerObject.vectors[ 0 ][ 2 ] ) ).length();
		var spacingY = ( new B3D.Vector3( headerObject.vectors[ 1 ][ 0 ], headerObject.vectors[ 1 ][ 1 ],
		headerObject.vectors[ 1 ][ 2 ] ) ).length();
		var spacingZ = ( new B3D.Vector3( headerObject.vectors[ 2 ][ 0 ], headerObject.vectors[ 2 ][ 1 ],
		headerObject.vectors[ 2 ][ 2 ] ) ).length();
		volume.spacing = [ spacingX, spacingY, spacingZ ];


		// Create IJKtoRAS matrix
		volume.matrix = new B3D.Matrix4();
		...
		volume.inverseMatrix = new B3D.Matrix4();
		volume.inverseMatrix.getInverse( volume.matrix );
		volume.RASDimensions = ( new B3D.Vector3( volume.xLength, volume.yLength, volume.zLength ) ).applyMatrix4( volume.matrix ).round().toArray().map( Math.abs );
