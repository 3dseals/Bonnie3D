...
B3D.PlayCanvasLoader = function ( manager ) {

	this.manager = ( manager !== undefined ) ? manager : B3D.DefaultLoadingManager;

};

B3D.PlayCanvasLoader.prototype = {

	constructor: B3D.PlayCanvasLoader,
	...
	parse: function ( json ) {
		...
		function parseVertices( data ) {
			...
			for ( var name in data ) {
				...
				attributes[ name ] = new B3D.BufferAttribute( array, size );
		...
		function parseMeshes( data ) {
			...
			geometry.setIndex( new B3D.Uint16BufferAttribute( data.indices, 1 ) );
		...
		function parseNodes( data ) {
			...
			if ( data._geometries !== undefined ) {
				...
				for ( var i = 0; i < data._geometries.length; i ++ ) {
					...
					object.add( new B3D.Mesh( geometry, material ) );
