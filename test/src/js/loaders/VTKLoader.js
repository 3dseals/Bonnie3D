...
B3D.VTKLoader = function( manager ) {

	this.manager = ( manager !== undefined ) ? manager : B3D.DefaultLoadingManager;

};

Object.assign( B3D.VTKLoader.prototype, B3D.EventDispatcher.prototype, {
	...
	parse: function ( data ) {
		...
		function parseASCII( data ) {
			...
			if ( stagger == 'point' ) {
				...
				geometry.setIndex( new B3D.BufferAttribute( new Uint32Array( indices ), 1 ) );
				geometry.addAttribute( 'position', new B3D.BufferAttribute( new Float32Array( positions ), 3 ) );

				if ( colors.length == positions.length ) {

					geometry.addAttribute( 'color', new B3D.BufferAttribute( new Float32Array( colors ), 3 ) );

				}

				if ( normals.length == positions.length ) {

					geometry.addAttribute( 'normal', new B3D.BufferAttribute( new Float32Array( normals ), 3 ) );
				...
				for ( var j = 0; j < numPoints; ++ j ) {
					...
					y = positions[ 3 * j + 1 ];
					z = positions[ 3 * j + 2 ];
					geometry.vertices.push( new B3D.Vector3( x, y, z ) );
				...
				for ( var i = 0; i < numTriangles; ++ i ) {
					...
					ib = indices[ 3 * i + 1 ];
					ic = indices[ 3 * i + 2 ];
					geometry.faces.push( new B3D.Face3( ia, ib, ic ) );
				...
				if ( colors.length == numTriangles * 3 ) {
					...
					for ( var i = 0; i < numTriangles; ++ i ) {
						...
						g = colors[ 3 * i + 1 ];
						b = colors[ 3 * i + 2 ];
						face.color = new B3D.Color().setRGB( r, g, b );
		...
		function parseBinary( data ) {
			...
			geometry.addAttribute( 'position', new B3D.BufferAttribute( points, 3 ) );
