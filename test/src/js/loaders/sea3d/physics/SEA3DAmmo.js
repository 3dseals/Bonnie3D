...
SEA3D.AMMO = {
	...
	init: function ( gravity, worldScale, broadphase ) {
		...
		console.log( "B3D.AMMO " + this.VERSION );
	...
	getTransformFromMatrix: function ( mtx ) {
		...
		var scl = B3D.SEA3D.VECBUF.setFromMatrixScale( mtx );
		mtx.scale( scl.set( 1 / scl.x, 1 / scl.y, 1 / scl.z ) );

		var quat = new B3D.Quaternion().setFromRotationMatrix( mtx );
	...
	getMatrixFromTransform: function ( transform ) {
		...
		var scale = new B3D.Vector3( 1, 1, 1 );

		return function ( transform, matrix ) {

			matrix = matrix || new B3D.Matrix4();
