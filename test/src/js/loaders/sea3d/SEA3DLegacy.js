...
Object.assign( B3D.SEA3D.prototype, {

	_onHead: B3D.SEA3D.prototype.onHead,
	_updateTransform: B3D.SEA3D.prototype.updateTransform,
	_readVertexAnimation: B3D.SEA3D.prototype.readVertexAnimation,
	_readGeometryBuffer: B3D.SEA3D.prototype.readGeometryBuffer,
	_readLine: B3D.SEA3D.prototype.readLine,
	_getAnimationType: B3D.SEA3D.prototype.getAnimationType,
	_readAnimation: B3D.SEA3D.prototype.readAnimation
...
B3D.SEA3D.prototype.isLegacy = function ( sea ) {
...
B3D.SEA3D.prototype.flipVec3 = function ( v ) {
...
B3D.SEA3D.prototype.expandJoints = function ( sea ) {
...
B3D.SEA3D.prototype.compressJoints = function ( sea ) {
...
B3D.SEA3D.prototype.flipIndexes = function ( v ) {
...
B3D.SEA3D.prototype.flipBoneMatrix = function () {

	var zero = new B3D.Vector3();

	return function ( mtx ) {

		var pos = B3D.SEA3D.VECBUF.setFromMatrixPosition( mtx );
		pos.z = - pos.z;

		mtx.setPosition( zero );
		mtx.multiplyMatrices( B3D.SEA3D.MTXBUF.makeRotationZ( B3D.Math.degToRad( 180 ) ), mtx );
		mtx.setPosition( pos );
...
B3D.SEA3D.prototype.flipScaleMatrix = function () {

	var pos = new B3D.Vector3();
	var qua = new B3D.Quaternion();
	var slc = new B3D.Vector3();
	...
	return function ( local, rotate, parent, parentRotate ) {
		...
		if ( rotate ) {
			...
			local.multiplyMatrices( local, B3D.SEA3D.MTXBUF.makeRotationZ( B3D.Math.degToRad( 180 ) ) );
...
B3D.SEA3D.prototype.flipDefaultAnimation = function () {

	var buf1 = new B3D.Matrix4();
	var buf2 = new B3D.Matrix4();

	var pos = new B3D.Vector3();
	var qua = new B3D.Quaternion();
	var slc = new B3D.Vector3();

	var to_pos = new B3D.Vector3();
	var to_qua = new B3D.Quaternion();
	var to_slc = new B3D.Vector3();
	...
	if ( this.isLegacy( sea ) ) {
		...
		switch ( sea.type ) {
			...
			case SEA3D.Animation.prototype.type:
				...
				if ( req.scope instanceof B3D.Object3D ) {
...
B3D.SEA3D.prototype.updateTransform = function () {

	var buf1 = new B3D.Matrix4();
	var identity = new B3D.Matrix4();
	...
	return function ( obj3d, sea ) {
		...
		if ( this.isLegacy( sea ) ) {
			...
			this.flipScaleMatrix(
				...
				obj3d.parent instanceof B3D.Bone
			);
			...
			buf1.scale( B3D.SEA3D.VECBUF.set( 1 / obj3d.scale.x, 1 / obj3d.scale.y, 1 / obj3d.scale.z ) );
...
B3D.SEA3D.prototype.readSkeleton = function () {

	var mtx_tmp_inv = new B3D.Matrix4(),
		mtx_local = new B3D.Matrix4(),
		mtx_parent = new B3D.Matrix4(),
		pos = new B3D.Vector3(),
		qua = new B3D.Quaternion();
...
B3D.SEA3D.prototype.readSkeletonAnimationLegacy = function () {

	var mtx_tmp_inv = new B3D.Matrix4(),
		mtx_local = new B3D.Matrix4(),
		mtx_global = new B3D.Matrix4(),
		mtx_parent = new B3D.Matrix4();
		...
		for ( var i = 0; i < sea.sequence.length; i ++ ) {
			...
			for ( var j = 0; j < numJoints; j ++ ) {
				...
				for ( var frame = start; frame < end; frame ++ ) {
					...
					mtx_local.makeRotationFromQuaternion( B3D.SEA3D.QUABUF.set( raw[ idx + 3 ], raw[ idx + 4 ], raw[ idx + 5 ], raw[ idx + 6 ] ) );
					mtx_local.setPosition( B3D.SEA3D.VECBUF.set( raw[ idx ], raw[ idx + 1 ], raw[ idx + 2 ] ) );
					...
					var posQ = B3D.SEA3D.VECBUF.setFromMatrixPosition( mtx_local );
					var newQ = B3D.SEA3D.QUABUF.setFromRotationMatrix( mtx_local );
...
B3D.SEA3D.prototype.readLines = function ( sea ) {
...
B3D.SEA3D.EXTENSIONS_LOADER.push( { setTypeRead: function () {
