...
if ( B3D.Float32BufferAttribute === undefined ) {

	B3D.Float32BufferAttribute = B3D.Float32Attribute;
...
B3D.SEA3D.MTXBUF = new B3D.Matrix4();
B3D.SEA3D.VECBUF = new B3D.Vector3();
B3D.SEA3D.QUABUF = new B3D.Quaternion();

B3D.SEA3D.BACKGROUND_COLOR = 0x333333;
B3D.SEA3D.HELPER_COLOR = 0x9AB9E5;
B3D.SEA3D.RTT_SIZE = 512;

B3D.SEA3D.prototype = Object.assign( Object.create( B3D.EventDispatcher.prototype ), {

	constructor: B3D.SEA3D,
...
Object.defineProperties( B3D.SEA3D.prototype, {
...
B3D.SEA3D.Domain = function ( id, objects, container ) {
	...
	this.events = new B3D.EventDispatcher();

};

Object.assign( B3D.SEA3D.Domain.prototype, {
	...
	runJSM: function ( target, script ) {
		...
		var include = {
			...
			source: new B3D.SEA3D.ScriptDomain( this, target instanceof B3D.SEA3D.Domain )
		};

		Object.freeze( include.source );

		B3D.SEA3D.ScriptHandler.add( include.source );
	...
	dispose: function () {
		...
		var i = B3D.SEA3D.EXTENSIONS_DOMAIN.length;

		while ( i -- ) {

			var domain = B3D.SEA3D.EXTENSIONS_DOMAIN[ i ];
...
B3D.SEA3D.DomainManager = function ( autoDisposeRootDomain ) {
...
Object.assign( B3D.SEA3D.DomainManager.prototype, {
...
B3D.SEA3D.ScriptDomain = function ( domain, root ) {

	domain = domain || new B3D.SEA3D.Domain();
	domain.add( this );

	var events = new B3D.EventDispatcher();
...
B3D.SEA3D.ScriptManager = function () {
...
B3D.SEA3D.ScriptHandler = new B3D.SEA3D.ScriptManager();

B3D.SEA3D.ScriptHandler.dispatchUpdate = function ( delta ) {
...
B3D.SEA3D.AnimationClip = function ( name, duration, tracks, repeat ) {

	B3D.AnimationClip.call( this, name, duration, tracks );

	this.repeat = repeat !== undefined ? repeat : true;

};

B3D.SEA3D.AnimationClip.fromClip = function ( clip, repeat ) {

	return new B3D.SEA3D.AnimationClip( clip.name, clip.duration, clip.tracks, repeat );

};

B3D.SEA3D.AnimationClip.prototype = Object.assign( Object.create( B3D.AnimationClip.prototype ), {

	constructor: B3D.SEA3D.AnimationClip
...
B3D.SEA3D.Animation = function ( clip, timeScale ) {
...
B3D.SEA3D.Animation.COMPLETE = "animationComplete";

B3D.SEA3D.Animation.prototype = Object.assign( Object.create( B3D.EventDispatcher.prototype ), {

	constructor: B3D.SEA3D.Animation,

	onComplete: function ( scope ) {

		this.dispatchEvent( { type: B3D.SEA3D.Animation.COMPLETE, target: this } );
...
Object.defineProperties( B3D.SEA3D.Animation.prototype, {
...
B3D.SEA3D.Animator = function ( clips, mixer ) {

	this.updateAnimations( clips, mixer );

	this.clone = function ( scope ) {

		return new this.constructor( this.clips, new B3D.AnimationMixer( scope ) ).copyFrom( this );

	}.bind( this );

};

Object.assign( B3D.SEA3D.Animator.prototype, {
	...
	updateAnimations: function ( clips, mixer ) {
		...
		if ( this.mixer ) B3D.SEA3D.AnimationHandler.remove( this );
	...
	addAnimation: function ( animation ) {
		...
		if ( animation instanceof B3D.AnimationClip ) {

			this.clips.push( animation );

			animation = new B3D.SEA3D.Animation( animation );
	...
	pause: function () {
		...
		if ( this.playing && this.currentAnimation ) {
			...
			B3D.SEA3D.AnimationHandler.remove( this );
	...
	resume: function () {
		...
		if ( ! this.playing && this.currentAnimation ) {
			...
			B3D.SEA3D.AnimationHandler.add( this );
	...
	play: function ( name, crossfade, offset, weight ) {
		...
		if ( animation == this.currentAnimation ) {
			...
			this.currentAnimationAction = this.mixer.clipAction( animation.clip ).setLoop( animation.repeat ? B3D.LoopRepeat : B3D.LoopOnce, Infinity ).reset();
	...
	stop: function () {
		...
		if ( this.playing ) B3D.SEA3D.AnimationHandler.remove( this );
	...
	playw: function ( name, weight ) {
		...
		if ( ! this.playing && ! this.paused ) B3D.SEA3D.AnimationHandler.add( this );
		...
		clip.setLoop( animation.repeat ? B3D.LoopRepeat : B3D.LoopOnce, Infinity ).reset();
...
B3D.SEA3D.Object3DAnimator = function ( clips, object3d ) {

	this.object3d = object3d;

	B3D.SEA3D.Animator.call( this, clips, new B3D.AnimationMixer( object3d ) );
...
B3D.SEA3D.Object3DAnimator.prototype = Object.assign( Object.create( B3D.SEA3D.Animator.prototype ), {

	constructor: B3D.SEA3D.Object3DAnimator,
	...
	stop: function () {
		...
		if ( this.currentAnimation ) {
			...
			if ( animate && this instanceof B3D.SEA3D.Object3DAnimator ) {
		...
		B3D.SEA3D.Animator.prototype.stop.call( this );

	},

	setRelative: function ( val ) {

		B3D.SEA3D.Animator.prototype.setRelative.call( this, val );

		this.object3d.setAnimator( this.relative );

		this.updateAnimations( this.clips, new B3D.AnimationMixer( this.relative ? this.object3d.animate : this.object3d ) );
...
B3D.SEA3D.CameraAnimator = function ( clips, object3d ) {

	B3D.SEA3D.Object3DAnimator.call( this, clips, object3d );

};

B3D.SEA3D.CameraAnimator.prototype = Object.assign( Object.create( B3D.SEA3D.Object3DAnimator.prototype ), {

	constructor: B3D.SEA3D.CameraAnimator
...
B3D.SEA3D.SoundAnimator = function ( clips, object3d ) {

	B3D.SEA3D.Object3DAnimator.call( this, clips, object3d );

};

B3D.SEA3D.SoundAnimator.prototype = Object.assign( Object.create( B3D.SEA3D.Object3DAnimator.prototype ), {

	constructor: B3D.SEA3D.SoundAnimator
...
B3D.SEA3D.LightAnimator = function ( clips, object3d ) {

	B3D.SEA3D.Object3DAnimator.call( this, clips, object3d );

};

B3D.SEA3D.LightAnimator.prototype = Object.assign( Object.create( B3D.SEA3D.Object3DAnimator.prototype ), {

	constructor: B3D.SEA3D.LightAnimator
...
B3D.SEA3D.Object3D = function ( ) {

	B3D.Object3D.call( this );

};

B3D.SEA3D.Object3D.prototype = Object.assign( Object.create( B3D.Object3D.prototype ), {

	constructor: B3D.SEA3D.Object3D,
	...
	setAnimator: function ( val ) {
		...
		if ( val ) {
			...
			this.animate = new B3D.Object3D();

			this.updateMatrixWorld = B3D.SEA3D.Object3D.prototype.updateAnimateMatrix;

		} else {

			delete this.animate;

			this.updateMatrixWorld = B3D.Object3D.prototype.updateMatrixWorld;
	...
	copy: function ( source ) {
		...
		B3D.Object3D.prototype.copy.call( this, source );
...
B3D.SEA3D.Dummy = function ( width, height, depth ) {

	this.width = width != undefined ? width : 100;
	this.height = height != undefined ? height : 100;
	this.depth = depth != undefined ? depth : 100;

	var geo = new B3D.BoxGeometry( this.width, this.height, this.depth, 1, 1, 1 );

	geo.computeBoundingBox();
	geo.computeBoundingSphere();

	B3D.Mesh.call( this, geo, B3D.SEA3D.Dummy.MATERIAL );

};

B3D.SEA3D.Dummy.MATERIAL = new B3D.MeshBasicMaterial( { wireframe: true, color: B3D.SEA3D.HELPER_COLOR } );

B3D.SEA3D.Dummy.prototype = Object.assign( Object.create( B3D.Mesh.prototype ), B3D.SEA3D.Object3D.prototype, {

	constructor: B3D.SEA3D.Dummy,
...
B3D.SEA3D.Mesh = function ( geometry, material ) {

	B3D.Mesh.call( this, geometry, material );

};

B3D.SEA3D.Mesh.prototype = Object.assign( Object.create( B3D.Mesh.prototype ), B3D.SEA3D.Object3D.prototype, {

	constructor: B3D.SEA3D.Mesh,
...
B3D.SEA3D.SkinnedMesh = function ( geometry, material, useVertexTexture ) {

	B3D.SkinnedMesh.call( this, geometry, material, useVertexTexture );

	this.updateAnimations( geometry.animations, new B3D.AnimationMixer( this ) );

};

B3D.SEA3D.SkinnedMesh.prototype = Object.assign( Object.create( B3D.SkinnedMesh.prototype ), B3D.SEA3D.Mesh.prototype, B3D.SEA3D.Animator.prototype, {

	constructor: B3D.SEA3D.SkinnedMesh,
	...
	copy: function ( source ) {
		...
		B3D.SkinnedMesh.prototype.copy.call( this, source );
...
B3D.SEA3D.VertexAnimationMesh = function ( geometry, material ) {
...
B3D.SEA3D.VertexAnimationMesh.prototype = Object.assign( Object.create( B3D.Mesh.prototype ), B3D.SEA3D.Mesh.prototype, B3D.SEA3D.Animator.prototype, {

	constructor: B3D.SEA3D.VertexAnimationMesh,
...
B3D.SEA3D.Camera = function ( fov, aspect, near, far ) {

	B3D.PerspectiveCamera.call( this, fov, aspect, near, far );

};

B3D.SEA3D.Camera.prototype = Object.assign( Object.create( B3D.PerspectiveCamera.prototype ), B3D.SEA3D.Object3D.prototype, {

	constructor: B3D.SEA3D.Camera,

	copy: function ( source ) {

		B3D.PerspectiveCamera.prototype.copy.call( this, source );
...
B3D.SEA3D.OrthographicCamera = function ( left, right, top, bottom, near, far ) {

	B3D.OrthographicCamera.call( this, left, right, top, bottom, near, far );

};

B3D.SEA3D.OrthographicCamera.prototype = Object.assign( Object.create( B3D.OrthographicCamera.prototype ), B3D.SEA3D.Object3D.prototype, {

	constructor: B3D.SEA3D.OrthographicCamera,

	copy: function ( source ) {

		B3D.OrthographicCamera.prototype.copy.call( this, source );
...
B3D.SEA3D.PointLight = function ( hex, intensity, distance, decay ) {

	B3D.PointLight.call( this, hex, intensity, distance, decay );

};

B3D.SEA3D.PointLight.prototype = Object.assign( Object.create( B3D.PointLight.prototype ), B3D.SEA3D.Object3D.prototype, {

	constructor: B3D.SEA3D.PointLight,

	copy: function ( source ) {

		B3D.PointLight.prototype.copy.call( this, source );
...
B3D.SEA3D.PointSound = function ( listener ) {

	B3D.PositionalAudio.call( this, listener );

};

B3D.SEA3D.PointSound.prototype = Object.assign( Object.create( B3D.PositionalAudio.prototype ), B3D.SEA3D.Object3D.prototype, {

	constructor: B3D.SEA3D.PointSound,

	copy: function ( source ) {

		B3D.PositionalAudio.prototype.copy.call( this, source );
...
B3D.SEA3D.Domain.prototype.getMesh = B3D.SEA3D.prototype.getMesh = function ( name ) {

	return this.objects[ "m3d/" + name ];

};

B3D.SEA3D.Domain.prototype.getDummy = B3D.SEA3D.prototype.getDummy = function ( name ) {

	return this.objects[ "dmy/" + name ];

};

B3D.SEA3D.Domain.prototype.getLine = B3D.SEA3D.prototype.getLine = function ( name ) {

	return this.objects[ "line/" + name ];

};

B3D.SEA3D.Domain.prototype.getSound3D = B3D.SEA3D.prototype.getSound3D = function ( name ) {

	return this.objects[ "sn3d/" + name ];

};

B3D.SEA3D.Domain.prototype.getMaterial = B3D.SEA3D.prototype.getMaterial = function ( name ) {

	return this.objects[ "mat/" + name ];

};

B3D.SEA3D.Domain.prototype.getLight = B3D.SEA3D.prototype.getLight = function ( name ) {

	return this.objects[ "lht/" + name ];

};

B3D.SEA3D.Domain.prototype.getGLSL = B3D.SEA3D.prototype.getGLSL = function ( name ) {

	return this.objects[ "glsl/" + name ];

};

B3D.SEA3D.Domain.prototype.getCamera = B3D.SEA3D.prototype.getCamera = function ( name ) {

	return this.objects[ "cam/" + name ];

};

B3D.SEA3D.Domain.prototype.getTexture = B3D.SEA3D.prototype.getTexture = function ( name ) {

	return this.objects[ "tex/" + name ];

};

B3D.SEA3D.Domain.prototype.getCubeMap = B3D.SEA3D.prototype.getCubeMap = function ( name ) {

	return this.objects[ "cmap/" + name ];

};

B3D.SEA3D.Domain.prototype.getJointObject = B3D.SEA3D.prototype.getJointObject = function ( name ) {

	return this.objects[ "jnt/" + name ];

};

B3D.SEA3D.Domain.prototype.getContainer3D = B3D.SEA3D.prototype.getContainer3D = function ( name ) {

	return this.objects[ "c3d/" + name ];

};

B3D.SEA3D.Domain.prototype.getSprite = B3D.SEA3D.prototype.getSprite = function ( name ) {

	return this.objects[ "m2d/" + name ];

};

B3D.SEA3D.Domain.prototype.getProperties = B3D.SEA3D.prototype.getProperties = function ( name ) {
...
B3D.SEA3D.prototype.isPowerOfTwo = function ( num ) {

	return num ? ( ( num & - num ) == num ) : false;

};

B3D.SEA3D.prototype.nearestPowerOfTwo = function ( num ) {

	return Math.pow( 2, Math.round( Math.log( num ) / Math.LN2 ) );

};

B3D.SEA3D.prototype.updateTransform = function ( obj3d, sea ) {

	var mtx = B3D.SEA3D.MTXBUF, vec = B3D.SEA3D.VECBUF;
...
B3D.SEA3D.prototype.toVector3 = function ( data ) {

	return new B3D.Vector3( data.x, data.y, data.z );

};

B3D.SEA3D.prototype.toFaces = function ( faces ) {
...
B3D.SEA3D.prototype.updateScene = function () {
...
B3D.SEA3D.prototype.addSceneObject = function ( sea, obj3d ) {
...
B3D.SEA3D.prototype.createObjectURL = function ( raw, mime ) {

	return ( window.URL || window.webkitURL ).createObjectURL( new Blob( [ raw ], { type: mime } ) );

};

B3D.SEA3D.prototype.bufferToTexture = function ( raw ) {

	return this.createObjectURL( raw, "image" );

};

B3D.SEA3D.prototype.bufferToSound = function ( raw ) {

	return this.createObjectURL( raw, "audio" );

};

B3D.SEA3D.prototype.parsePath = function ( url ) {
...
B3D.SEA3D.prototype.addDefaultAnimation = function ( sea, animatorClass ) {
...
B3D.SEA3D.prototype.readGeometryBuffer = function ( sea ) {

	var geo = new B3D.BufferGeometry();
	...
	geo.setIndex( new B3D.BufferAttribute( sea.indexes, 1 ) );
	geo.addAttribute( 'position', new B3D.BufferAttribute( sea.vertex, 3 ) );

	if ( sea.uv ) {

		geo.addAttribute( 'uv', new B3D.BufferAttribute( sea.uv[ 0 ], 2 ) );
		if ( sea.uv.length > 1 ) geo.addAttribute( 'uv2', new B3D.BufferAttribute( sea.uv[ 1 ], 2 ) );

	}

	if ( sea.normal ) geo.addAttribute( 'normal', new B3D.BufferAttribute( sea.normal, 3 ) );
	else geo.computeVertexNormals();

	if ( sea.tangent4 ) geo.addAttribute( 'tangent', new B3D.BufferAttribute( sea.tangent4, 4 ) );

	if ( sea.color ) geo.addAttribute( 'color', new B3D.BufferAttribute( sea.color[ 0 ], sea.numColor ) );

	if ( sea.joint ) {

		geo.addAttribute( 'skinIndex', new B3D.Float32BufferAttribute( sea.joint, sea.jointPerVertex ) );
		geo.addAttribute( 'skinWeight', new B3D.Float32BufferAttribute( sea.weight, sea.jointPerVertex ) );
...
B3D.SEA3D.prototype.readDummy = function ( sea ) {

	var dummy = new B3D.SEA3D.Dummy( sea.width, sea.height, sea.depth );
	dummy.name = sea.name;
	...
	this.addDefaultAnimation( sea, B3D.SEA3D.Object3DAnimator );
...
B3D.SEA3D.prototype.readLine = function ( sea ) {
	...
	geo.addAttribute( 'position', new B3D.Float32BufferAttribute( sea.vertex, 3 ) );

	var line = new B3D.Line( geo, new B3D.LineBasicMaterial( { color: B3D.SEA3D.HELPER_COLOR, linewidth: 3 } ) );
	line.name = sea.name;
...
B3D.SEA3D.prototype.readContainer3D = function ( sea ) {

	var container = new B3D.SEA3D.Object3D();
...
B3D.SEA3D.prototype.readSprite = function ( sea ) {
	...
	if ( sea.material ) {
		...
		if ( ! sea.material.tag.sprite ) {
			...
			mat = sea.material.tag.sprite = new B3D.SpriteMaterial();
...
B3D.SEA3D.prototype.readMesh = function ( sea ) {
	...
	if ( sea.material ) {
		...
		if ( sea.material.length > 1 ) {
			...
			for ( i = 0; i < sea.material.length; i ++ ) {
				...
				mats[ i ].vertexColors = sea.geometry.color ? B3D.VertexColors : B3D.NoColors;

			}

//			mat = new B3D.MultiMaterial( mats );
			mat = mats;
			...
			mat.vertexColors = sea.geometry.color ? B3D.VertexColors : B3D.NoColors;
	...
	if ( skeleton ) {
		...
		mesh = new B3D.SEA3D.SkinnedMesh( geo, mat, this.config.useVertexTexture );
		...
		mesh = new B3D.SEA3D.VertexAnimationMesh( geo, mat, vertexAnimation.frameRate );
		...
		mesh = new B3D.SEA3D.Mesh( geo, mat );
...
B3D.SEA3D.prototype.readSoundPoint = function ( sea ) {

	if ( ! this.audioListener ) {

		 this.audioListener = new B3D.AudioListener();
	...
	var sound3d = new B3D.SEA3D.PointSound( this.audioListener );

	new B3D.AudioLoader().load( sea.sound.tag, function ( buffer ) {
	...
	this.addDefaultAnimation( sea, B3D.SEA3D.SoundAnimator );
...
B3D.SEA3D.prototype.readCubeRender = function ( sea ) {

	var cube = new B3D.CubeCamera( 0.1, 5000, B3D.SEA3D.RTT_SIZE );
...
B3D.SEA3D.prototype.readTexture = function ( sea ) {

	var image = new Image(),
		texture = new B3D.Texture();

	texture.name = sea.name;
	texture.wrapS = texture.wrapT = B3D.RepeatWrapping;
	texture.flipY = false;
	texture.image = image;
...
B3D.SEA3D.prototype.readCubeMap = function ( sea ) {

	var faces = this.toFaces( sea.faces ), texture = new B3D.CubeTexture( [] );

	var loaded = 0;

	texture.name = sea.name;
	texture.flipY = false;
	texture.format = B3D.RGBFormat;
...
B3D.SEA3D.prototype.readSound = function ( sea ) {
...
B3D.SEA3D.prototype.readScriptURL = function ( sea ) {
...
B3D.SEA3D.prototype.readTextureURL = function ( sea ) {

	var texture = new B3D.TextureLoader().load( this.parsePath( sea.url ) );
...
B3D.SEA3D.prototype.readCubeMapURL = function ( sea ) {
	...
	if ( format == "hdr" ) {
		...
		var usePMREM = B3D.PMREMGenerator != null;

		this.file.resume = ! usePMREM;

		texture = new B3D.HDRCubeTextureLoader().load( B3D.UnsignedByteType, faces, function ( texture ) {

			if ( usePMREM ) {

				var pmremGenerator = new B3D.PMREMGenerator( texture );
				pmremGenerator.update( this.config.renderer );

				var pmremCubeUVPacker = new B3D.PMREMCubeUVPacker( pmremGenerator.cubeLods );
		...
		texture = new B3D.CubeTextureLoader().load( faces );
...
B3D.SEA3D.prototype.getJSMList = function ( target, scripts ) {
...
B3D.SEA3D.prototype.readJavaScriptMethod = function ( sea ) {
...
B3D.SEA3D.prototype.readGLSL = function ( sea ) {
...
B3D.SEA3D.prototype.materialTechnique =
( function () {
	...
	techniques.onComplete = function ( mat, sea ) {
		...
		if ( sea.alpha < 1 || mat.blending > B3D.NormalBlending ) {
	...
	function ( mat, tech, sea ) {
		...
		mat.map.wrapS = mat.map.wrapT = sea.repeat ? B3D.RepeatWrapping : B3D.ClampToEdgeWrapping;
	...
	function ( mat, tech ) {
		...
		mat.envMap.mapping = B3D.CubeReflectionMapping;
		mat.combine = B3D.MixOperation;
	...
	function ( mat, tech ) {
		...
		mat.envMap = tech.texture.tag;
		mat.envMap.mapping = B3D.SphericalReflectionMapping;
		mat.combine = B3D.MixOperation;
	...
	function ( mat, tech ) {
		...
		mat.envMap = tech.texture.tag;
		mat.envMap.mapping = B3D.CubeRefractionMapping;
	...
	function ( mat, tech, sea ) {
		...
		mat.alphaMap.wrapS = mat.alphaMap.wrapT = sea.repeat ? B3D.RepeatWrapping : B3D.ClampToEdgeWrapping;
...
B3D.SEA3D.prototype.setBlending = function ( mat, blendMode ) {
	...
	switch ( blendMode ) {
		...
		case "add":
			...
			mat.blending = B3D.AdditiveBlending;

			break;

		case "subtract":

			mat.blending = B3D.SubtractiveBlending;

			break;

		case "multiply":

			mat.blending = B3D.MultiplyBlending;

			break;

		case "screen":

			mat.blending = B3D.CustomBlending;
			mat.blendSrc = B3D.OneFactor;
			mat.blendDst = B3D.OneMinusSrcColorFactor;
			mat.blendEquation = B3D.AddEquation;
...
B3D.SEA3D.prototype.readMaterial = function ( sea ) {
	...
	mat.side = sea.bothSides ? B3D.DoubleSide : B3D.FrontSide;
...
B3D.SEA3D.prototype.readPointLight = function ( sea ) {

	var light = new B3D.SEA3D.PointLight( sea.color, sea.multiplier * this.config.multiplier );
	light.name = sea.name;
	...
	this.addDefaultAnimation( sea, B3D.SEA3D.LightAnimator );
...
B3D.SEA3D.prototype.readHemisphereLight = function ( sea ) {

	var light = new B3D.HemisphereLight( sea.color, sea.secondColor, sea.multiplier * this.config.multiplier );
...
B3D.SEA3D.prototype.readAmbientLight = function ( sea ) {

	var light = new B3D.AmbientLight( sea.color, sea.multiplier * this.config.multiplier );
	light.name = sea.name;
...
B3D.SEA3D.prototype.readDirectionalLight = function ( sea ) {

	var light = new B3D.DirectionalLight( sea.color, sea.multiplier * this.config.multiplier );
	light.name = sea.name;
...
B3D.SEA3D.prototype.readCamera = function ( sea ) {

	var camera = new B3D.SEA3D.Camera( sea.fov );
	...
	this.addDefaultAnimation( sea, B3D.SEA3D.CameraAnimator );
...
B3D.SEA3D.prototype.readOrthographicCamera = function ( sea ) {
	...
	var camera = new B3D.SEA3D.OrthographicCamera( - width, width, height, - height );
	camera.name = sea.name;
...
B3D.SEA3D.prototype.readSkeletonLocal = function ( sea ) {
...
B3D.SEA3D.prototype.readJointObject = function ( sea ) {
...
B3D.SEA3D.prototype.readMorpher = function ( sea ) {
	...
	for ( var i = 0; i < sea.node.length; i ++ ) {
		...
		attribs.position[ i ] = new B3D.Float32BufferAttribute( node.vertex, 3 );
		attribs.position[ i ].name = node.name;

		if ( node.normal ) {

			attribs.normal = attribs.normal || [];
			attribs.normal[ i ] = new B3D.Float32BufferAttribute( node.normal, 3 );
...
B3D.SEA3D.prototype.readAnimation = function ( sea ) {
	...
	for ( var i = 0; i < sea.sequence.length; i ++ ) {
		...
		for ( var j = 0; j < sea.dataList.length; j ++ ) {
			...
			var anm = sea.dataList[ j ],
				...
				intrpl = seq.intrpl ? B3D.InterpolateLinear : false,
				name = null;
			...
			switch ( anm.type ) {
				...
				case SEA3D.Stream.DECIMAL:
					...
					tracks.push( new B3D.VectorKeyframeTrack( name, times, values, intrpl ) );
				...
				case SEA3D.Stream.VECTOR4D:
					...
					tracks.push( new B3D.QuaternionKeyframeTrack( name, times, values, intrpl ) );
				...
				case SEA3D.Stream.UINT24:
					...
					tracks.push( new B3D.VectorKeyframeTrack( name, times, values, intrpl ) );//ColorKeyframeTrack
		...
		animations.push( new B3D.SEA3D.AnimationClip( seq.name, - 1, tracks, seq.repeat ) );
...
B3D.SEA3D.prototype.readSkeletonAnimation = function ( sea, skl ) {
	...
	for ( var i = 0; i < sea.sequence.length; i ++ ) {
		...
		animations.push( B3D.SEA3D.AnimationClip.fromClip( B3D.AnimationClip.parseAnimation( animation, skl.tag ), seq.repeat ) );
...
B3D.SEA3D.prototype.readVertexAnimation = function ( sea ) {
	...
	for ( i = 0, l = sea.frame.length; i < l; i ++ ) {
		...
		attribs.position[ i ] = new B3D.Float32BufferAttribute( frame.vertex, 3 );

		if ( frame.normal ) {

			attribs.normal = attribs.normal || [];
			attribs.normal[ i ] = new B3D.Float32BufferAttribute( frame.normal, 3 );
	...
	for ( i = 0; i < sea.sequence.length; i ++ ) {
		...
		animations.push( B3D.SEA3D.AnimationClip.fromClip( B3D.AnimationClip.CreateFromMorphTargetSequence( seq.name, seqTargets, sea.frameRate ), seq.repeat ) );
...
B3D.SEA3D.prototype.getAnimationType = function ( req ) {
...
B3D.SEA3D.prototype.applyEnvironment = function ( envMap ) {

	for ( var j = 0, l = this.materials.length; j < l; ++ j ) {

		var mat = this.materials[ j ];

		if ( mat instanceof B3D.MeshStandardMaterial ) {
...
B3D.SEA3D.prototype.readActions = function ( sea ) {
	...
	for ( var i = 0; i < sea.actions.length; i ++ ) {
		...
		switch ( act.kind ) {
			...
			case SEA3D.Actions.ENVIRONMENT_COLOR:
				...
				this.background.color = new B3D.Color( act.color );
...
B3D.SEA3D.prototype.onProgress = function ( e ) {

	e.status = e.type;
	e.progress = e.loaded / e.total;
	e.type = B3D.SEA3D.Event.PROGRESS;

	this.dispatchEvent( e );

};

B3D.SEA3D.prototype.onLoadProgress = function ( e ) {

	e.type = B3D.SEA3D.Event.LOAD_PROGRESS;
	this.dispatchEvent( e );
...
B3D.SEA3D.prototype.onDownloadProgress = function ( e ) {

	e.type = B3D.SEA3D.Event.DOWNLOAD_PROGRESS;
	this.dispatchEvent( e );
...
B3D.SEA3D.prototype.onComplete = function ( e ) {

	e.type = B3D.SEA3D.Event.COMPLETE;
	this.dispatchEvent( e );

};

B3D.SEA3D.prototype.onCompleteObject = function ( e ) {

	e.type = B3D.SEA3D.Event.OBJECT_COMPLETE;
	this.dispatchEvent( e );

};

B3D.SEA3D.prototype.onParseProgress = function ( e ) {

	e.type = B3D.SEA3D.Event.PARSE_PROGRESS;
	this.dispatchEvent( e );

};

B3D.SEA3D.prototype.onParseComplete = function ( e ) {

	e.type = B3D.SEA3D.Event.PARSE_COMPLETE;
	this.dispatchEvent( e );

};

B3D.SEA3D.prototype.onError = function ( e ) {

	e.type = B3D.SEA3D.Event.ERROR;
	this.dispatchEvent( e );
...
B3D.SEA3D.prototype.createDomain = function () {

	return this.domain = new B3D.SEA3D.Domain(
		this.config.id,
		this.objects = {},
...
B3D.SEA3D.prototype.clone = function ( config, onParseComplete, onParseProgress ) {

	if ( ! this.file.isDone() ) throw new Error( "Previous parse is not completed." );

	this.config.container = config && config.container !== undefined ? config.container : new B3D.Object3D();
...
B3D.SEA3D.prototype.loadConfig = function ( config ) {
...
B3D.SEA3D.prototype.parse = function ( onParseComplete, onParseProgress ) {
	...
	var i = B3D.SEA3D.EXTENSIONS_LOADER.length;

	while ( i -- ) {

		var loader = B3D.SEA3D.EXTENSIONS_LOADER[ i ];
...
B3D.SEA3D.prototype.onHead = function ( args ) {
...
B3D.SEA3D.EXTENSIONS_LOADER = [];
B3D.SEA3D.EXTENSIONS_DOMAIN = [];

B3D.SEA3D.prototype.setTypeRead = function () {
...
B3D.SEA3D.prototype.load = function ( data ) {
