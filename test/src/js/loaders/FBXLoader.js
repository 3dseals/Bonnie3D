...
( function () {
	 ...
	 * a B3D.Group.
	 * @param {B3D.LoadingManager} manager - Loading Manager for loader to use.
	 */
	B3D.FBXLoader = function ( manager ) {

		this.manager = ( manager !== undefined ) ? manager : B3D.DefaultLoadingManager;

	};

	Object.assign( B3D.FBXLoader.prototype, {

		/**
		 * Loads an ASCII/Binary FBX file from URL and parses into a B3D.Group.
		 * B3D.Group will have an animations property of AnimationClips
		 * of the different animations exported with the FBX.
		 * @param {string} url - URL of the FBX file.
		 * @param {function(B3D.Group):void} onLoad - Callback for when FBX file is loaded and parsed.
		...
		/**
		 * Parses an ASCII/Binary FBX file and returns a B3D.Group.
		 * B3D.Group will have an animations property of AnimationClips
		...
		parse: function ( FBXBuffer, resourceDirectory ) {
			...
			var textures = parseTextures( FBXTree, new B3D.TextureLoader( this.manager ).setPath( resourceDirectory ), images, connections );
	...
	function parseTexture( textureNode, loader, imageMap, connections ) {
		...
		texture.wrapS = valueU === 0 ? B3D.RepeatWrapping : B3D.ClampToEdgeWrapping;
		texture.wrapT = valueV === 0 ? B3D.RepeatWrapping : B3D.ClampToEdgeWrapping;
	 ...
	 * @param {Map<number, B3D.Texture>} textureMap
	 * @param {Map<number, {parents: {ID: number, relationship: string}[], children: {ID: number, relationship: string}[]}>} connections
	 * @returns {Map<number, B3D.Material>}
	 */
	...
	/**
	 * Takes information from Material node and returns a generated B3D.Material
	 * @param {FBXMaterialNode} materialNode
	 * @param {Map<number, B3D.Texture>} textureMap
	 * @param {Map<number, {parents: {ID: number, relationship: string}[], children: {ID: number, relationship: string}[]}>} connections
	 * @returns {B3D.Material}
	 */
	function parseMaterial( materialNode, textureMap, connections ) {
		...
		switch ( type.toLowerCase() ) {
			...
			case 'phong':
				material = new B3D.MeshPhongMaterial();
				break;
			case 'lambert':
				material = new B3D.MeshLambertMaterial();
				break;
			default:
				console.warn( 'No implementation given for material type ' + type + ' in FBXLoader.js.  Defaulting to basic material' );
				material = new B3D.MeshBasicMaterial( { color: 0x3300ff } );
				break;
	 ...
	 */
	/**
	 * @typedef {{color: B3D.Color=, specular: B3D.Color=, shininess: number=, emissive: B3D.Color=, emissiveIntensity: number=, opacity: number=, transparent: boolean=, map: B3D.Texture=}} B3DMaterialParameterPack
	 */
	/**
	 * @param {FBXMaterialProperties} properties
	 * @param {Map<number, B3D.Texture>} textureMap
	 * @param {{ID: number, relationship: string}[]} childrenRelationships
	 * @returns {B3DMaterialParameterPack}
	 */
	 ...
	 * @param {Map<number, {parents: {ID: number, relationship: string}[], children: {ID: number, relationship: string}[]}>} connections
	 * @returns {Map<number, {map: Map<number, {FBX_ID: number, indices: number[], weights: number[], transform: number[], transformLink: number[], linkMode: string}>, array: {FBX_ID: number, indices: number[], weights: number[], transform: number[], transformLink: number[], linkMode: string}[], skeleton: B3D.Skeleton|null}>}
	 */
	 ...
	 * @returns {{map: Map<number, {FBX_ID: number, indices: number[], weights: number[], transform: number[], transformLink: number[], linkMode: string}>, array: {FBX_ID: number, indices: number[], weights: number[], transform: number[], transformLink: number[], linkMode: string}[], skeleton: B3D.Skeleton|null}}
	 */
	...
	/**
	 * Generates Buffer geometries from geometry information in FBXTree, and generates map of B3D.BufferGeometries
	 * @param {{Objects: {subNodes: {Geometry: Object.<number, FBXGeometryNode}}}} FBXTree
	 * @param {Map<number, {parents: {ID: number, relationship: string}[], children: {ID: number, relationship: string}[]}>} connections
	 * @param {Map<number, {map: Map<number, {FBX_ID: number, indices: number[], weights: number[], transform: number[], transformLink: number[], linkMode: string}>, array: {FBX_ID: number, indices: number[], weights: number[], transform: number[], transformLink: number[], linkMode: string}[], skeleton: B3D.Skeleton|null}>} deformers
	 * @returns {Map<number, B3D.BufferGeometry>}
	 */
	...
	function genGeometry( geometryNode, deformer ) {
		...
		geo.addAttribute( 'position', new B3D.Float32BufferAttribute( bufferInfo.vertexBuffer, 3 ) );

		if ( bufferInfo.normalBuffer.length > 0 ) {

			geo.addAttribute( 'normal', new B3D.Float32BufferAttribute( bufferInfo.normalBuffer, 3 ) );

		}
		if ( bufferInfo.uvBuffer.length > 0 ) {

			geo.addAttribute( 'uv', new B3D.Float32BufferAttribute( bufferInfo.uvBuffer, 2 ) );

		}
		if ( subNodes.LayerElementColor ) {

			geo.addAttribute( 'color', new B3D.Float32BufferAttribute( bufferInfo.colorBuffer, 3 ) );

		}

		if ( deformer ) {

			geo.addAttribute( 'skinIndex', new B3D.Float32BufferAttribute( bufferInfo.skinIndexBuffer, 4 ) );

			geo.addAttribute( 'skinWeight', new B3D.Float32BufferAttribute( bufferInfo.skinWeightBuffer, 4 ) );
	...
	function parseNurbsGeometry( geometryNode ) {
		...
		if ( B3D.NURBSCurve === undefined ) {

			console.error( "B3D.FBXLoader relies on B3D.NURBSCurve for any nurbs present in the model.  Nurbs will show up as empty geometry." );
			return new B3D.BufferGeometry();
		...
		for ( var i = 0, l = pointsValues.length; i < l; i += 4 ) {
			...
			controlPoints.push( new B3D.Vector4().fromArray( pointsValues, i ) );
		...
		var curve = new B3D.NURBSCurve( degree, knots, controlPoints, startKnot, endKnot );
	 ...
	 * @param {Map<number, {parents: {ID: number, relationship: string}[], children: {ID: number, relationship: string}[]}>} connections
	 * @param {Map<number, {map: Map<number, {FBX_ID: number, indices: number[], weights: number[], transform: number[], transformLink: number[], linkMode: string}>, array: {FBX_ID: number, indices: number[], weights: number[], transform: number[], transformLink: number[], linkMode: string}[], skeleton: B3D.Skeleton|null}>} deformers
	 * @param {Map<number, B3D.BufferGeometry>} geometryMap
	 * @param {Map<number, B3D.Material>} materialMap
	 * @returns {B3D.Group}
	 */
	function parseScene( FBXTree, connections, deformers, geometryMap, materialMap ) {

		var sceneGraph = new B3D.Group();

		var ModelNode = FBXTree.Objects.subNodes.Model;

		/**
		 * @type {Array.<B3D.Object3D>}
		 */
		var modelArray = [];

		/**
		 * @type {Map.<number, B3D.Object3D>}
		 */
		...
		for ( var nodeID in ModelNode ) {
			...
			if ( ! model ) {
				...
				switch ( node.attrType ) {
					...
					case "Mesh":
						/**
						 * @type {?B3D.BufferGeometry}
						 */
						var geometry = null;

						/**
						 * @type {B3D.MultiMaterial|B3D.Material}
						 */
						var material = null;

						/**
						 * @type {Array.<B3D.Material>}
						 */
						var materials = [];
						...
						if ( 'color' in geometry.attributes ) {
							...
							for ( var materialIndex = 0, numMaterials = materials.length; materialIndex < numMaterials; ++materialIndex ) {
								...
								materials[ materialIndex ].vertexColors = B3D.VertexColors;
						...
						if ( geometry.FBX_Deformer ) {
							...
							model = new B3D.SkinnedMesh( geometry, material );

						} else {

							model = new B3D.Mesh( geometry, material );
					...
					case "NurbsCurve":
						...
						material = new B3D.LineBasicMaterial( { color: 0x3300ff, linewidth: 5 } );
						model = new B3D.Line( geometry, material );
						break;

					default:
						model = new B3D.Object3D();
						break;
		...
		for ( var modelArrayIndex = 0, modelArrayLength = modelArray.length; modelArrayIndex < modelArrayLength; ++ modelArrayIndex ) {
			...
			if ( 'PreRotation' in node.properties ) {
				...
				var preRotations = new B3D.Euler().setFromVector3( parseVector3( node.properties.PreRotation ).multiplyScalar( DEG2RAD ), 'ZYX' );
				preRotations = new B3D.Quaternion().setFromEuler( preRotations );
				var currentRotation = new B3D.Quaternion().setFromEuler( model.rotation );
		...
		for ( var FBX_ID in deformers ) {
			...
			deformer.skeleton = new B3D.Skeleton( deformer.bones );
		...
		tmpMap.forEach( function ( curveNode ) {
			...
			if ( curveNode.attr === 'R' ) {
				...
				if ( curveNode.preRotations !== null ) {
					...
					var preRotations = new B3D.Euler().setFromVector3( curveNode.preRotations, 'ZYX' );
					preRotations = new B3D.Quaternion().setFromEuler( preRotations );
					var frameRotation = new B3D.Euler();
					var frameRotationQuaternion = new B3D.Quaternion();
	 ...
	 * @param {{skeleton: { bones: B3D.Bone[]}}} group
	 */
	function addAnimations( group, animations ) {
		...
		for ( var key in stacks ) {
			...
			group.animations.push( B3D.AnimationClip.parseAnimation( animationData, bones ) );
	...
	function Vertex() {
		 ...
		 * @type {B3D.Vector3}
		 */
		this.position = new B3D.Vector3();

		/**
		 * Normal of the vertex
		 * @type {B3D.Vector3}
		 */
		this.normal = new B3D.Vector3();

		/**
		 * UV coordinates of the vertex.
		 * @type {B3D.Vector2}
		 */
		this.uv = new B3D.Vector2();

		/**
		 * Color of the vertex
		 * @type {B3D.Vector3}
		 */
		this.color = new B3D.Vector3();

		/**
		 * Indices of the bones vertex is influenced by.
		 * @type {B3D.Vector4}
		 */
		this.skinIndices = new B3D.Vector4( 0, 0, 0, 0 );

		/**
		 * Weights that each bone influences the vertex.
		 * @type {B3D.Vector4}
		 */
		this.skinWeights = new B3D.Vector4( 0, 0, 0, 0 );
	...
	function Triangle() {
		...
		/**
		 * @type {{position: B3D.Vector3, normal: B3D.Vector3, uv: B3D.Vector2, skinIndices: B3D.Vector4, skinWeights: B3D.Vector4}[]}
		 */
		this.vertices = [];
	...
	function Face() {
		...
		/**
		 * @type {{vertices: {position: B3D.Vector3, normal: B3D.Vector3, uv: B3D.Vector2, skinIndices: B3D.Vector4, skinWeights: B3D.Vector4}[]}[]}
		 */
		this.triangles = [];
	...
	function Geometry() {
		...
		/**
		 * @type {{triangles: {vertices: {position: B3D.Vector3, normal: B3D.Vector3, uv: B3D.Vector2, skinIndices: B3D.Vector4, skinWeights: B3D.Vector4}[]}[], materialIndex: number}[]}
		 */
		this.faces = [];
	...
	function parseVector3( property ) {
		...
		return new B3D.Vector3().fromArray( property.value );
	...
	function parseColor( property ) {
		...
		return new B3D.Color().fromArray( property.value );

	}

	function parseMatrixArray( floatString ) {

		return new B3D.Matrix4().fromArray( parseFloatArray( floatString ) );
