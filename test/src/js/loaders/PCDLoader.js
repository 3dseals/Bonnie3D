 ...
 *
 * Description: A B3D loader for PCD ascii and binary files.
 *
 * Limitations: Compressed binary files are not supported.
 *
 */

B3D.PCDLoader = function ( manager ) {
...
B3D.PCDLoader.prototype = {

	constructor: B3D.PCDLoader,
	...
	parse: function ( data, url ) {
		...
		if ( PCDheader.data == "binary_compressed" ) {
			...
			console.error( 'B3D.PCDLoader: binary_compressed files are not supported' );
			return;
		...
		if ( position != false )
			geometry.addAttribute( 'position', new B3D.BufferAttribute( position, 3 ) );
		if ( color != false )
			geometry.addAttribute( 'color', new B3D.BufferAttribute( color, 3 ) );
		if ( normal != false )
			geometry.addAttribute( 'normal', new B3D.BufferAttribute( normal, 3 ) );

		geometry.computeBoundingSphere();

		var material = new B3D.PointsMaterial( { size: 0.005, vertexColors: ! ( color == false ) } );
		if ( color == false )
			material.color.setHex( Math.random() * 0xffffff );

		var mesh = new B3D.Points( geometry, material );
