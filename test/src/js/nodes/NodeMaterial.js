...
B3D.NodeMaterial = function( vertex, fragment ) {

	B3D.ShaderMaterial.call( this );

	this.vertex = vertex || new B3D.RawNode( new B3D.PositionNode( B3D.PositionNode.PROJECTION ) );
	this.fragment = fragment || new B3D.RawNode( new B3D.ColorNode( 0xFF0000 ) );

};

B3D.NodeMaterial.types = {
	t : 'sampler2D',
	tc : 'samplerCube',
	bv1 : 'bool',
	iv1 : 'int',
...
B3D.NodeMaterial.addShortcuts = function( proto, prop, list ) {
...
B3D.NodeMaterial.prototype = Object.create( B3D.ShaderMaterial.prototype );
B3D.NodeMaterial.prototype.constructor = B3D.NodeMaterial;

B3D.NodeMaterial.prototype.updateFrame = function( delta ) {
...
B3D.NodeMaterial.prototype.build = function() {
	...
	var builder = new B3D.NodeBuilder( this );
	...
	this.transparent = this.requestAttribs.transparent || this.blending > B3D.NormalBlending;
...
B3D.NodeMaterial.prototype.define = function( name, value ) {

	this.defines[ name ] = value == undefined ? 1 : value;

};

B3D.NodeMaterial.prototype.isDefined = function( name ) {

	return this.defines[ name ] != undefined;

};

B3D.NodeMaterial.prototype.mergeUniform = function( uniforms ) {
...
B3D.NodeMaterial.prototype.createUniform = function( type, value, ns, needsUpdate ) {
...
B3D.NodeMaterial.prototype.getVertexTemp = function( uuid, type, ns ) {
...
B3D.NodeMaterial.prototype.getFragmentTemp = function( uuid, type, ns ) {
...
B3D.NodeMaterial.prototype.getVar = function( uuid, type, ns ) {
...
B3D.NodeMaterial.prototype.getAttribute = function( name, type ) {
...
B3D.NodeMaterial.prototype.getIncludes = function() {
...
B3D.NodeMaterial.prototype.addVertexPars = function( code ) {

	this.vertexPars += code + '\n';

};

B3D.NodeMaterial.prototype.addFragmentPars = function( code ) {

	this.fragmentPars += code + '\n';

};

B3D.NodeMaterial.prototype.addVertexCode = function( code ) {

	this.vertexCode += code + '\n';

};

B3D.NodeMaterial.prototype.addFragmentCode = function( code ) {

	this.fragmentCode += code + '\n';

};

B3D.NodeMaterial.prototype.addVertexNode = function( code ) {

	this.vertexNode += code + '\n';

};

B3D.NodeMaterial.prototype.clearVertexNode = function() {
...
B3D.NodeMaterial.prototype.addFragmentNode = function( code ) {

	this.fragmentNode += code + '\n';

};

B3D.NodeMaterial.prototype.clearFragmentNode = function() {
...
B3D.NodeMaterial.prototype.getCodePars = function( pars, prefix ) {
	...
	for ( var i = 0, l = pars.length; i < l; ++ i ) {
		...
		if ( parsType == 't' && parsValue instanceof B3D.CubeTexture ) parsType = 'tc';

		var type = B3D.NodeMaterial.types[ parsType ];
...
B3D.NodeMaterial.prototype.createVertexUniform = function( type, value, ns, needsUpdate ) {
...
B3D.NodeMaterial.prototype.createFragmentUniform = function( type, value, ns, needsUpdate ) {
...
B3D.NodeMaterial.prototype.getDataNode = function( uuid ) {

	return this.nodeData[ uuid ] = this.nodeData[ uuid ] || {};

};

B3D.NodeMaterial.prototype.include = function( builder, node, parent, source ) {

	var includes;

	node = typeof node === 'string' ? B3D.NodeLib.get( node ) : node;

	if ( node instanceof B3D.FunctionNode ) {

		includes = this.functions[ builder.shader ] = this.functions[ builder.shader ] || [];

	} else if ( node instanceof B3D.ConstNode ) {
	...
	if ( node instanceof B3D.FunctionNode && parent && includes[ parent.name ] && includes[ parent.name ].deps.indexOf( node ) == - 1 ) {
